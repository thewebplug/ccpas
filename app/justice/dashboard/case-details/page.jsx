"use client";
import Menu from "@/app/components/menu";
import Image from "next/image";
import { useState } from "react";
import CaseFileModal from "../case-file/page";

export default function CreateCase() {
  const [menuActive, setMenuActive] = useState(null)

  // ...

  const [modalIsOpen, setModalIsOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState(null);

  const openModal = (item) => {
    setSelectedItem(item);
    setModalIsOpen(true);
  };

  const closeModal = () => {
    setModalIsOpen(false);
    setSelectedItem(null);
  };


  //...


  const handleModalClose = (e) => {
    if (e.target.classList.contains("case-details__modal")) {
      setModalOpen(false);
    }
  };
  const handleUpload = async (item) => {
    setLoading(true);
    setTimeout(() => {
      setDoc(item);
      setLoading(false);
    }, 5000);
  };

  useEffect(() => {
    console.log("loading", loading);
  }, [loading]);

  const handleStage1 = () => {
    if (!!doc) {
      setModaleStage(2);
    } else {
      alert("Please Upload Certified Court Order For Expungement");
    }
  };
  const handleStage2 = () => {
    setModaleStage(3);
  };
  const handleStage3 = () => {
  setModaleStage(4)
  };
  const handleStage4 = () => {
  window.location.href ="/justice/expunge-doc"
  };
  return (
    <div className="case-details">
      <div className="case-details__menu">
        <div className="case-details__menu__flex">
          <svg
            width="101"
            height="100"
            viewBox="0 0 101 100"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <circle cx="50.7269" cy="50.4105" r="49.5706" fill="#FFE7EE" />
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
              d="M94.4587 65.5838H90.7676C84.5768 81.7774 68.893 93.2776 50.5233 93.2776C32.1535 93.2776 16.4698 81.7774 10.279 65.5838H6.58789C12.9378 83.729 30.2107 96.7432 50.5233 96.7432C70.8358 96.7432 88.1088 83.729 94.4587 65.5838ZM94.4583 34.8301H90.7671C84.576 18.6371 68.8926 7.13745 50.5233 7.13745C32.154 7.13745 16.4706 18.6371 10.2794 34.8301H6.5883C12.9386 16.6856 30.2112 3.67188 50.5233 3.67188C70.8354 3.67188 88.108 16.6856 94.4583 34.8301Z"
              fill="#B1241D"
            />
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
              d="M78.115 65.5883H23.209C28.5555 75.2415 38.8455 81.7767 50.662 81.7767C62.4784 81.7767 72.7685 75.2415 78.115 65.5883ZM77.8845 34.8343H23.4395C28.8477 25.4058 39.0131 19.0547 50.662 19.0547C62.3108 19.0547 72.4763 25.4058 77.8845 34.8343Z"
              fill="#B1241D"
            />
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
              d="M88.5473 65.5834H87.2456C81.2388 79.8546 67.1237 89.8743 50.6682 89.8743C34.2128 89.8743 20.0976 79.8546 14.0908 65.5834H12.7892C18.8647 80.5348 33.5354 91.0769 50.6682 91.0769C67.8011 91.0769 82.4718 80.5348 88.5473 65.5834ZM88.5474 34.8296H87.2457C81.239 20.5583 67.1237 10.5385 50.6682 10.5385C34.2127 10.5385 20.0975 20.5583 14.0907 34.8296H12.7891C18.8646 19.8781 33.5353 9.33594 50.6682 9.33594C67.8012 9.33594 82.4719 19.8781 88.5474 34.8296Z"
              fill="#B1241D"
            />
            <path
              d="M50.6281 74.8307L52.295 76.0437C52.5593 76.2402 52.9184 75.9759 52.8168 75.6642L52.1798 73.7059L53.8265 72.5336C54.0975 72.3438 53.962 71.9169 53.63 71.9169H51.6106L50.9533 69.8705C50.8517 69.5588 50.4112 69.5588 50.3096 69.8705L49.6455 71.9169H47.6262C47.2942 71.9169 47.1586 72.3438 47.4297 72.5336L49.0763 73.7059L48.4393 75.6642C48.3377 75.9759 48.6968 76.2334 48.9611 76.0437L50.6281 74.8307Z"
              fill="white"
            />
            <path
              d="M50.632 29.9088L52.2989 31.1218C52.5632 31.3183 52.9224 31.054 52.8207 30.7423L52.1837 28.784L53.8304 27.6117C54.1014 27.422 53.9659 26.9951 53.6339 26.9951H51.6145L50.9572 24.9486C50.8556 24.6369 50.4151 24.6369 50.3135 24.9486L49.6494 26.9951H47.6301C47.2981 26.9951 47.1625 27.422 47.4336 27.6117L49.0802 28.784L48.4433 30.7423C48.3416 31.054 48.7007 31.3115 48.965 31.1218L50.632 29.9088Z"
              fill="white"
            />
            <path
              d="M93.6652 60.042V43.1812H90.4336V40.3711H99.988V43.1812H96.7564V60.042H93.6652Z"
              fill="#B1241D"
            />
            <path
              d="M81.041 60.042V40.3711H89.4714V43.1812H84.1322V48.38H88.3754V51.1901H84.1322V57.2318H89.4714V60.042H81.041Z"
              fill="#B1241D"
            />
            <path
              d="M69.7051 60.0459V40.375H74.2856C77.4891 40.375 78.8661 41.8644 78.8661 44.8993V46.1077C78.8661 48.1309 78.2198 49.4236 76.8428 49.9856V50.0418C78.3884 50.5196 78.8942 51.9808 78.8942 54.2008V57.6573C78.8942 58.6127 78.9223 59.3152 79.2314 60.0459H76.0841C75.9155 59.5401 75.803 59.2309 75.803 57.6292V54.0322C75.803 52.2056 75.2691 51.6155 73.8641 51.6155H72.7962V60.0459H69.7051ZM72.7962 48.8054H73.9203C75.0724 48.8054 75.7749 48.2996 75.7749 46.7259V45.2084C75.7749 43.8034 75.2972 43.1851 74.2013 43.1851H72.7962V48.8054Z"
              fill="#B1241D"
            />
            <path
              d="M63.3044 60.3227C60.3256 60.3227 58.752 58.5243 58.752 55.4893V44.9233C58.752 41.8883 60.3256 40.0898 63.3044 40.0898C66.2831 40.0898 67.8568 41.8883 67.8568 44.9233V47.0028H64.9342V44.7266C64.9342 43.4339 64.316 42.9 63.3887 42.9C62.4613 42.9 61.8431 43.4339 61.8431 44.7266V55.7141C61.8431 57.0068 62.4613 57.5126 63.3887 57.5126C64.316 57.5126 64.9342 57.0068 64.9342 55.7141V52.7073H67.8568V55.4893C67.8568 58.5243 66.2831 60.3227 63.3044 60.3227Z"
              fill="#B1241D"
            />
            <path
              d="M48.7402 60.042V40.3711H57.1706V43.1812H51.8314V48.38H56.0747V51.1901H51.8314V57.2318H57.1706V60.042H48.7402Z"
              fill="#B1241D"
            />
            <path
              d="M42.3024 60.3227C39.2675 60.3227 37.75 58.5243 37.75 55.3769V54.2529H40.6725V55.6017C40.6725 57.0068 41.2908 57.5126 42.2743 57.5126C43.2578 57.5126 43.8761 57.0068 43.8761 55.5455C43.8761 53.9157 43.2578 52.8759 41.2346 51.1055C38.6492 48.8293 37.8062 47.2276 37.8062 44.9795C37.8062 41.8883 39.408 40.0898 42.4148 40.0898C45.4216 40.0898 46.911 41.8883 46.911 45.0357V45.8506H43.9885V44.839C43.9885 43.4339 43.4265 42.9 42.4429 42.9C41.4594 42.9 40.8973 43.4339 40.8973 44.7828C40.8973 46.2159 41.5437 47.2557 43.567 49.026C46.1523 51.3022 46.9672 52.8759 46.9672 55.3207C46.9672 58.5243 45.3373 60.3227 42.3024 60.3227Z"
              fill="#B1241D"
            />
            <path
              d="M22.9434 60.042L22.9434 40.3711H27.4958C30.5869 40.3711 32.1044 42.0853 32.1044 45.2326V47.1716C32.1044 50.3189 30.5869 52.0331 27.4958 52.0331H26.0345L26.0345 60.042H22.9434ZM26.0345 49.223H27.4958C28.4793 49.223 29.0132 48.7734 29.0132 47.3683V45.0359C29.0132 43.6308 28.4793 43.1812 27.4958 43.1812H26.0345V49.223Z"
              fill="#B1241D"
            />
            <path
              d="M16.1617 60.3227C13.1267 60.3227 11.4688 58.5243 11.4688 55.3769L11.4688 45.0357C11.4688 41.8883 13.1267 40.0898 16.1617 40.0898C19.1966 40.0898 20.8546 41.8883 20.8546 45.0357V55.3769C20.8546 58.5243 19.1966 60.3227 16.1617 60.3227ZM16.1617 57.5126C17.1452 57.5126 17.7634 56.9787 17.7634 55.5736V44.839C17.7634 43.4339 17.1452 42.9 16.1617 42.9C15.1781 42.9 14.5599 43.4339 14.5599 44.839L14.5599 55.5736C14.5599 56.9787 15.1781 57.5126 16.1617 57.5126Z"
              fill="#B1241D"
            />
            <path
              d="M3.98164 60.0459L3.98164 43.1851H0.75L0.75 40.375H10.3044V43.1851H7.07278L7.07278 60.0459H3.98164Z"
              fill="#B1241D"
            />
            <path
              d="M30.4716 25.0061C29.6842 25.8005 28.4233 25.7826 27.5934 24.9602C26.7675 24.1416 26.7423 22.8848 27.5297 22.0903C28.2665 21.347 29.3389 21.2561 30.16 21.875L29.5325 22.5082C29.0611 22.1736 28.47 22.2114 28.0724 22.6126C27.593 23.0964 27.6509 23.8009 28.2015 24.3466C28.7561 24.8962 29.465 24.9518 29.9445 24.4681C30.3421 24.0669 30.3669 23.4755 30.0319 23.0033L30.6595 22.3701C31.2936 23.1934 31.2083 24.2627 30.4716 25.0061Z"
              fill="black"
            />
            <path
              d="M34.2583 21.8061C33.3198 22.4641 32.0857 22.2473 31.4023 21.2725C30.7188 20.2977 30.9419 19.0727 31.8805 18.4147C32.8235 17.7534 34.0558 17.958 34.7392 18.9328C35.4227 19.9076 35.2014 21.1448 34.2583 21.8061ZM33.826 21.1894C34.4245 20.7698 34.447 20.0304 34.0274 19.4319C33.6078 18.8334 32.9113 18.6117 32.3128 19.0313C31.7188 19.4477 31.6899 20.1781 32.1096 20.7766C32.5292 21.3751 33.2321 21.6059 33.826 21.1894Z"
              fill="black"
            />
            <path
              d="M39.3205 18.8654L36.4985 17.1186L37.6687 19.6339L36.9709 19.9586L35.2937 16.3538L36.1171 15.9707L38.8004 17.6294L37.6886 15.2396L38.3915 14.9126L40.0686 18.5174L39.3205 18.8654Z"
              fill="black"
            />
            <path
              d="M42.2147 17.8232L41.4064 18.0579L40.2978 14.2398L42.7652 13.5233L42.969 14.2253L41.3099 14.707L41.5894 15.6695L42.9879 15.2634L43.1902 15.9601L41.7916 16.3662L42.2147 17.8232Z"
              fill="black"
            />
            <path
              d="M45.9084 16.9668L45.0794 17.1123L44.3921 13.1963L45.2212 13.0508L45.9084 16.9668Z"
              fill="black"
            />
            <path
              d="M48.7696 16.6543L47.3592 16.723L47.1659 12.7518L48.5763 12.6832C49.8373 12.6218 50.6399 13.3921 50.6961 14.5481C50.7524 15.704 50.003 16.5943 48.7696 16.6543ZM48.0419 13.4354L48.1646 15.9575L48.6901 15.9319C49.481 15.8934 49.8734 15.4142 49.8333 14.5901C49.7948 13.7992 49.3582 13.3714 48.5673 13.4099L48.0419 13.4354Z"
              fill="black"
            />
            <path
              d="M54.7869 16.1825L54.7109 16.9094L52.0509 16.6312L52.4645 12.6769L55.086 12.9511L55.01 13.6781L53.2256 13.4914L53.1386 14.323L54.7577 14.4924L54.6823 15.2139L53.0631 15.0445L52.964 15.9918L54.7869 16.1825Z"
              fill="black"
            />
            <path
              d="M58.4222 17.7723L57.3731 14.6235L56.6614 17.3049L55.9175 17.1075L56.9374 13.2647L57.8151 13.4976L58.8142 16.4898L59.4903 13.9422L60.2396 14.1411L59.2197 17.9839L58.4222 17.7723Z"
              fill="black"
            />
            <path
              d="M62.1667 19.195L61.3979 18.8523L62.7076 15.9137L61.5645 15.4043L61.8733 14.7114L64.9333 16.0751L64.6245 16.7681L63.4764 16.2564L62.1667 19.195Z"
              fill="black"
            />
            <path
              d="M65.0328 20.7331L64.3087 20.3039L66.336 16.8838L67.0601 17.313L65.0328 20.7331Z"
              fill="black"
            />
            <path
              d="M68.3923 23.1206L68.679 22.2637L67.4586 21.3163L66.6951 21.8031L66.0259 21.2836L69.645 19.0599L70.3711 19.6236L69.1052 23.6741L68.3923 23.1206ZM69.4269 20.0542L68.0576 20.933L68.9062 21.5918L69.4269 20.0542Z"
              fill="black"
            />
            <path
              d="M71.6844 26.34L69.9142 24.4778L72.7957 21.7385L73.3757 22.3485L71.0439 24.5651L72.2342 25.8173L71.6844 26.34Z"
              fill="black"
            />
            <path
              d="M70.3663 75.2713C71.1537 74.4768 72.4146 74.4947 73.2445 75.3172C74.0704 76.1358 74.0956 77.3926 73.3082 78.187C72.5714 78.9303 71.499 79.0213 70.6778 78.4023L71.3054 77.7691C71.7768 78.1038 72.3679 78.0659 72.7655 77.6647C73.2449 77.181 73.187 76.4764 72.6364 75.9307C72.0818 75.3811 71.3729 75.3255 70.8934 75.8093C70.4958 76.2104 70.471 76.8018 70.8059 77.2741L70.1784 77.9073C69.5443 77.0839 69.6296 76.0146 70.3663 75.2713Z"
              fill="black"
            />
            <path
              d="M66.5795 78.4713C67.5181 77.8132 68.7521 78.03 69.4356 79.0048C70.1191 79.9796 69.896 81.2046 68.9574 81.8627C68.0144 82.5239 66.7821 82.3194 66.0986 81.3446C65.4152 80.3698 65.6365 79.1325 66.5795 78.4713ZM67.0119 79.0879C66.4134 79.5075 66.3908 80.247 66.8105 80.8455C67.2301 81.4439 67.9266 81.6657 68.5251 81.2461C69.119 80.8296 69.148 80.0992 68.7283 79.5007C68.3087 78.9023 67.6058 78.6715 67.0119 79.0879Z"
              fill="black"
            />
            <path
              d="M61.5174 81.4119L64.3394 83.1588L63.1692 80.6434L63.867 80.3187L65.5442 83.9235L64.7208 84.3066L62.0375 82.6479L63.1493 85.0377L62.4464 85.3648L60.7693 81.76L61.5174 81.4119Z"
              fill="black"
            />
            <path
              d="M58.6232 82.4541L59.4315 82.2194L60.5401 86.0376L58.0727 86.754L57.8689 86.0521L59.528 85.5703L59.2485 84.6078L57.85 85.0139L57.6477 84.3173L59.0463 83.9112L58.6232 82.4541Z"
              fill="black"
            />
            <path
              d="M54.9295 83.3105L55.7585 83.165L56.4457 87.081L55.6167 87.2265L54.9295 83.3105Z"
              fill="black"
            />
            <path
              d="M52.0683 83.623L53.4787 83.5544L53.672 87.5255L52.2616 87.5942C51.0006 87.6556 50.198 86.8852 50.1418 85.7293C50.0855 84.5733 50.8349 83.6831 52.0683 83.623ZM52.796 86.8419L52.6733 84.3198L52.1478 84.3454C51.3569 84.3839 50.9644 84.8632 51.0046 85.6873C51.0431 86.4782 51.4797 86.906 52.2706 86.8675L52.796 86.8419Z"
              fill="black"
            />
            <path
              d="M46.0509 84.0949L46.127 83.3679L48.787 83.6462L48.3734 87.6004L45.7519 87.3262L45.8279 86.5992L47.6123 86.7859L47.6993 85.9543L46.0801 85.7849L46.1556 85.0635L47.7748 85.2328L47.8739 84.2856L46.0509 84.0949Z"
              fill="black"
            />
            <path
              d="M42.4157 82.5051L43.4648 85.6538L44.1765 82.9724L44.9204 83.1699L43.9005 87.0127L43.0228 86.7797L42.0237 83.7875L41.3476 86.3351L40.5983 86.1362L41.6182 82.2934L42.4157 82.5051Z"
              fill="black"
            />
            <path
              d="M38.6712 81.0824L39.44 81.425L38.1303 84.3636L39.2734 84.873L38.9646 85.566L35.9046 84.2022L36.2134 83.5093L37.3615 84.021L38.6712 81.0824Z"
              fill="black"
            />
            <path
              d="M35.8051 79.5442L36.5291 79.9734L34.5019 83.3936L33.7778 82.9644L35.8051 79.5442Z"
              fill="black"
            />
            <path
              d="M32.4456 77.1567L32.1589 78.0137L33.3792 78.9611L34.1428 78.4742L34.812 78.9938L31.1929 81.2175L30.4668 80.6538L31.7327 76.6032L32.4456 77.1567ZM31.411 80.2231L32.7803 79.3443L31.9317 78.6855L31.411 80.2231Z"
              fill="black"
            />
            <path
              d="M29.1535 73.9374L30.9237 75.7995L28.0421 78.5389L27.4622 77.9288L29.794 75.7122L28.6036 74.46L29.1535 73.9374Z"
              fill="black"
            />
          </svg>

          <div className="case-details__menu__flex__menu-group">
            <div
              className="case-details__menu__flex__menu-group__item"
              onClick={() =>
                (window.location.href = "/justice/dashboard/edit-case")
              }
            >
              <div>Edit/Append Case File</div>
              <svg
                width="48"
                height="48"
                viewBox="0 0 48 48"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <circle
                  cx="24"
                  cy="24"
                  r="23.5"
                  fill="white"
                  stroke="#A8A8A8"
                />
                <path
                  d="M33 34H15C14.59 34 14.25 33.66 14.25 33.25C14.25 32.84 14.59 32.5 15 32.5H33C33.41 32.5 33.75 32.84 33.75 33.25C33.75 33.66 33.41 34 33 34Z"
                  fill="#292D32"
                />
                <path
                  d="M31.0196 15.4816C29.0796 13.5416 27.1796 13.4916 25.1896 15.4816L23.9796 16.6916C23.8796 16.7916 23.8396 16.9516 23.8796 17.0916C24.6396 19.7416 26.7596 21.8616 29.4096 22.6216C29.4496 22.6316 29.4896 22.6416 29.5296 22.6416C29.6396 22.6416 29.7396 22.6016 29.8196 22.5216L31.0196 21.3116C32.0096 20.3316 32.4896 19.3816 32.4896 18.4216C32.4996 17.4316 32.0196 16.4716 31.0196 15.4816Z"
                  fill="#292D32"
                />
                <path
                  d="M27.6103 23.5308C27.3203 23.3908 27.0403 23.2508 26.7703 23.0908C26.5503 22.9608 26.3403 22.8208 26.1303 22.6708C25.9603 22.5608 25.7603 22.4008 25.5703 22.2408C25.5503 22.2308 25.4803 22.1708 25.4003 22.0908C25.0703 21.8108 24.7003 21.4508 24.3703 21.0508C24.3403 21.0308 24.2903 20.9608 24.2203 20.8708C24.1203 20.7508 23.9503 20.5508 23.8003 20.3208C23.6803 20.1708 23.5403 19.9508 23.4103 19.7308C23.2503 19.4608 23.1103 19.1908 22.9703 18.9108C22.9491 18.8654 22.9286 18.8202 22.9088 18.7753C22.7612 18.442 22.3265 18.3446 22.0688 18.6023L16.3403 24.3308C16.2103 24.4608 16.0903 24.7108 16.0603 24.8808L15.5203 28.7108C15.4203 29.3908 15.6103 30.0308 16.0303 30.4608C16.3903 30.8108 16.8903 31.0008 17.4303 31.0008C17.5503 31.0008 17.6703 30.9908 17.7903 30.9708L21.6303 30.4308C21.8103 30.4008 22.0603 30.2808 22.1803 30.1508L27.9016 24.4295C28.1612 24.1699 28.0633 23.7245 27.7257 23.5804C27.6877 23.5642 27.6492 23.5476 27.6103 23.5308Z"
                  fill="#292D32"
                />
              </svg>
            </div>
            <div
              className="case-details__menu__flex__menu-group__item"
              onClick={() => setModalOpen(true)}
            >
              <div>Expunge Case File</div>
              <svg
                width="48"
                height="48"
                viewBox="0 0 48 48"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <circle
                  cx="24"
                  cy="24"
                  r="23.5"
                  fill="white"
                  stroke="#A8A8A8"
                />
                <path
                  d="M33.0697 17.23C31.4597 17.07 29.8497 16.95 28.2297 16.86V16.85L28.0097 15.55C27.8597 14.63 27.6397 13.25 25.2997 13.25H22.6797C20.3497 13.25 20.1297 14.57 19.9697 15.54L19.7597 16.82C18.8297 16.88 17.8997 16.94 16.9697 17.03L14.9297 17.23C14.5097 17.27 14.2097 17.64 14.2497 18.05C14.2897 18.46 14.6497 18.76 15.0697 18.72L17.1097 18.52C22.3497 18 27.6297 18.2 32.9297 18.73C32.9597 18.73 32.9797 18.73 33.0097 18.73C33.3897 18.73 33.7197 18.44 33.7597 18.05C33.7897 17.64 33.4897 17.27 33.0697 17.23Z"
                  fill="#292D32"
                />
                <path
                  d="M31.2297 20.14C30.9897 19.89 30.6597 19.75 30.3197 19.75H17.6797C17.3397 19.75 16.9997 19.89 16.7697 20.14C16.5397 20.39 16.4097 20.73 16.4297 21.08L17.0497 31.34C17.1597 32.86 17.2997 34.76 20.7897 34.76H27.2097C30.6997 34.76 30.8398 32.87 30.9497 31.34L31.5697 21.09C31.5897 20.73 31.4597 20.39 31.2297 20.14ZM25.6597 29.75H22.3297C21.9197 29.75 21.5797 29.41 21.5797 29C21.5797 28.59 21.9197 28.25 22.3297 28.25H25.6597C26.0697 28.25 26.4097 28.59 26.4097 29C26.4097 29.41 26.0697 29.75 25.6597 29.75ZM26.4997 25.75H21.4997C21.0897 25.75 20.7497 25.41 20.7497 25C20.7497 24.59 21.0897 24.25 21.4997 24.25H26.4997C26.9097 24.25 27.2497 24.59 27.2497 25C27.2497 25.41 26.9097 25.75 26.4997 25.75Z"
                  fill="#292D32"
                />
              </svg>
            </div>
            <div
              className="case-details__menu__flex__menu-group__item"
              onClick={() =>
                menuActive === "transfer"
                  ? setMenuActive(null)
                  : setMenuActive("transfer")
              }
            >
              <div>Transfer Case Files</div>
              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M17.9197 8.17969H11.6897H6.07975C5.11975 8.17969 4.63975 9.33969 5.31975 10.0197L10.4997 15.1997C11.3297 16.0297 12.6797 16.0297 13.5097 15.1997L15.4797 13.2297L18.6897 10.0197C19.3597 9.33969 18.8797 8.17969 17.9197 8.17969Z"
                  fill="#292D32"
                />
              </svg>

              {menuActive === "transfer" && (
                <Menu
                  items={[
                    { name: "Out-side Agency" },
                    { name: "Within Agency" },
                  ]}
                />
              )}
            </div>
            <div
              className="case-details__menu__flex__menu-group__item"
              onClick={() =>
                menuActive === "classify"
                  ? setMenuActive(null)
                  : setMenuActive("classify")
              }
            >
              <div>Classify Case File</div>
              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M17.9197 8.17969H11.6897H6.07975C5.11975 8.17969 4.63975 9.33969 5.31975 10.0197L10.4997 15.1997C11.3297 16.0297 12.6797 16.0297 13.5097 15.1997L15.4797 13.2297L18.6897 10.0197C19.3597 9.33969 18.8797 8.17969 17.9197 8.17969Z"
                  fill="#292D32"
                />
              </svg>


  </div>
  <div className="case-details__menu__flex__menu-group__item">
    <div>Expunge Case File</div>
    <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
<circle cx="24" cy="24" r="23.5" fill="white" stroke="#A8A8A8"/>
<path d="M33.0697 17.23C31.4597 17.07 29.8497 16.95 28.2297 16.86V16.85L28.0097 15.55C27.8597 14.63 27.6397 13.25 25.2997 13.25H22.6797C20.3497 13.25 20.1297 14.57 19.9697 15.54L19.7597 16.82C18.8297 16.88 17.8997 16.94 16.9697 17.03L14.9297 17.23C14.5097 17.27 14.2097 17.64 14.2497 18.05C14.2897 18.46 14.6497 18.76 15.0697 18.72L17.1097 18.52C22.3497 18 27.6297 18.2 32.9297 18.73C32.9597 18.73 32.9797 18.73 33.0097 18.73C33.3897 18.73 33.7197 18.44 33.7597 18.05C33.7897 17.64 33.4897 17.27 33.0697 17.23Z" fill="#292D32"/>
<path d="M31.2297 20.14C30.9897 19.89 30.6597 19.75 30.3197 19.75H17.6797C17.3397 19.75 16.9997 19.89 16.7697 20.14C16.5397 20.39 16.4097 20.73 16.4297 21.08L17.0497 31.34C17.1597 32.86 17.2997 34.76 20.7897 34.76H27.2097C30.6997 34.76 30.8398 32.87 30.9497 31.34L31.5697 21.09C31.5897 20.73 31.4597 20.39 31.2297 20.14ZM25.6597 29.75H22.3297C21.9197 29.75 21.5797 29.41 21.5797 29C21.5797 28.59 21.9197 28.25 22.3297 28.25H25.6597C26.0697 28.25 26.4097 28.59 26.4097 29C26.4097 29.41 26.0697 29.75 25.6597 29.75ZM26.4997 25.75H21.4997C21.0897 25.75 20.7497 25.41 20.7497 25C20.7497 24.59 21.0897 24.25 21.4997 24.25H26.4997C26.9097 24.25 27.2497 24.59 27.2497 25C27.2497 25.41 26.9097 25.75 26.4997 25.75Z" fill="#292D32"/>
</svg>


  </div>
  <div className="case-details__menu__flex__menu-group__item"
  onClick={() => menuActive === "transfer" ? setMenuActive(null): setMenuActive("transfer")}
  > 
    <div>Transfer Case Files</div>
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M17.9197 8.17969H11.6897H6.07975C5.11975 8.17969 4.63975 9.33969 5.31975 10.0197L10.4997 15.1997C11.3297 16.0297 12.6797 16.0297 13.5097 15.1997L15.4797 13.2297L18.6897 10.0197C19.3597 9.33969 18.8797 8.17969 17.9197 8.17969Z" fill="#292D32"/>
</svg>

 <div>
    {menuActive === "transfer" &&
    <Menu items={[{name: "Out-side Agency"}, {name: "Within Agency"}]} onItemClick={openModal} />
    }
</div>

<CaseFileModal isOpen={modalIsOpen} onRequestClose={closeModal} item={selectedItem} />


  </div>
  <div className="case-details__menu__flex__menu-group__item"
  onClick={() => menuActive === "classify" ? setMenuActive(null): setMenuActive("classify")}
  >
    <div>Classify Case File</div>
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M17.9197 8.17969H11.6897H6.07975C5.11975 8.17969 4.63975 9.33969 5.31975 10.0197L10.4997 15.1997C11.3297 16.0297 12.6797 16.0297 13.5097 15.1997L15.4797 13.2297L18.6897 10.0197C19.3597 9.33969 18.8797 8.17969 17.9197 8.17969Z" fill="#292D32"/>
</svg>

{menuActive === "classify" &&
<Menu items={[{name: "Top secret"}, {name: "Secret"}, {name: "Official"}, {name: "Public"}, {name: "General"}]} />
}
  </div>
</div>
        </div>

        <div className="case-details__menu__name">
          Case File Created by: Ahmed Olaniyi
        </div>
      </div>
      <form className="case-details__inner">
        <div className="case-details__inner__grid">
          <div className="case-details__inner__grid__input">
            <label htmlFor="">Case Number</label>
            {/* <input type="text" className="" placeholder="Type case number" /> */}
            <div className="case-details__inner__grid__input__filled">
              PF00458
            </div>
          </div>
          <div className="case-details__inner__grid__input">
            <label htmlFor="">Offense Category:</label>
            {/* <select name="" id="">
            <option value="">Hello</option>
          </select> */}
            <div className="case-details__inner__grid__input__filled">
              Criminal Case
            </div>
          </div>
          <div className="case-details__inner__grid__input">
            <label htmlFor="">FMoJ Dept:</label>
            {/* <select name="" id="">
            <option value="">Hello</option>
          </select> */}
            <div className="case-details__inner__grid__input__filled">
              EFCC ABUJA
            </div>
          </div>

          <div className="case-details__inner__grid__input">
            <label htmlFor="">Assigned Justice Officer:</label>
            {/* <div className="case-details__inner__grid__input__flex">
            <input type="text" placeholder="Select assigned officer" />
            <svg
              width="21"
              height="20"
              viewBox="0 0 21 20"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clip-path="url(#clip0_4643_76588)">
                <path
                  d="M15.4497 14.9489C12.7161 17.6825 8.28392 17.6825 5.55025 14.9489C2.81658 12.2152 2.81658 7.78303 5.55025 5.04936C8.28392 2.31569 12.7161 2.31569 15.4497 5.04936C18.1834 7.78303 18.1834 12.2152 15.4497 14.9489ZM15.4497 14.9489L21 20.4991"
                  stroke="#7E92A2"
                  stroke-width="1.5"
                />
              </g>
              <defs>
                <clipPath id="clip0_4643_76588">
                  <rect
                    width="20"
                    height="20"
                    fill="white"
                    transform="translate(0.5)"
                  />
                </clipPath>
              </defs>
            </svg>
          </div> */}

            <div className="case-details__inner__grid__input__contact">
              Ahmed Aisha
              <svg
                width="20"
                height="20"
                viewBox="0 0 20 20"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M14.542 18.9577C13.6003 18.9577 12.6087 18.7327 11.5837 18.2993C10.5837 17.8743 9.57533 17.291 8.59199 16.5827C7.61699 15.866 6.67533 15.066 5.78366 14.191C4.90033 13.2993 4.10033 12.3577 3.39199 11.391C2.67533 10.391 2.10033 9.39102 1.69199 8.42435C1.25866 7.39102 1.04199 6.39102 1.04199 5.44935C1.04199 4.79935 1.15866 4.18268 1.38366 3.60768C1.61699 3.01602 1.99199 2.46602 2.50033 1.99102C3.14199 1.35768 3.87533 1.04102 4.65866 1.04102C4.98366 1.04102 5.31699 1.11602 5.60033 1.24935C5.92533 1.39935 6.20033 1.62435 6.40033 1.92435L8.33366 4.64935C8.50866 4.89102 8.64199 5.12435 8.73366 5.35768C8.84199 5.60768 8.90033 5.85768 8.90033 6.09935C8.90033 6.41602 8.80866 6.72435 8.63366 7.01602C8.50866 7.24102 8.31699 7.48268 8.07533 7.72435L7.50866 8.31602C7.51699 8.34102 7.52533 8.35768 7.53366 8.37435C7.63366 8.54935 7.83366 8.84935 8.21699 9.29935C8.62533 9.76602 9.00866 10.191 9.39199 10.5827C9.88366 11.066 10.292 11.4493 10.6753 11.766C11.1503 12.166 11.4587 12.366 11.642 12.4577L11.6253 12.4993L12.2337 11.8993C12.492 11.641 12.742 11.4493 12.9837 11.3243C13.442 11.041 14.0253 10.991 14.6087 11.2327C14.8253 11.3243 15.0587 11.4493 15.3087 11.6243L18.0753 13.591C18.3837 13.7993 18.6087 14.066 18.742 14.3827C18.867 14.6993 18.9253 14.991 18.9253 15.2827C18.9253 15.6827 18.8337 16.0827 18.6587 16.4577C18.4837 16.8327 18.267 17.1577 17.992 17.4577C17.517 17.9827 17.0003 18.3577 16.4003 18.5993C15.8253 18.8327 15.2003 18.9577 14.542 18.9577ZM4.65866 2.29102C4.20033 2.29102 3.77533 2.49102 3.36699 2.89102C2.98366 3.24935 2.71699 3.64102 2.55033 4.06602C2.37533 4.49935 2.29199 4.95768 2.29199 5.44935C2.29199 6.22435 2.47533 7.06602 2.84199 7.93268C3.21699 8.81602 3.74199 9.73268 4.40866 10.6493C5.07533 11.566 5.83366 12.4577 6.66699 13.2993C7.50033 14.1243 8.40033 14.891 9.32533 15.566C10.2253 16.2243 11.1503 16.7577 12.067 17.141C13.492 17.7493 14.8253 17.891 15.9253 17.4327C16.3503 17.2577 16.7253 16.991 17.067 16.6077C17.2587 16.3994 17.4087 16.1743 17.5337 15.9077C17.6337 15.6993 17.6837 15.4827 17.6837 15.266C17.6837 15.1327 17.6587 14.9993 17.592 14.8493C17.567 14.7993 17.517 14.7077 17.3587 14.5993L14.592 12.6327C14.4253 12.516 14.2753 12.4327 14.1337 12.3743C13.9503 12.2993 13.8753 12.2243 13.592 12.3993C13.4253 12.4827 13.2753 12.6077 13.1087 12.7743L12.4753 13.3993C12.1503 13.716 11.6503 13.791 11.267 13.6493L11.042 13.5493C10.7003 13.366 10.3003 13.0827 9.85866 12.7077C9.45866 12.366 9.02533 11.966 8.50033 11.4493C8.09199 11.0327 7.68366 10.591 7.25866 10.0993C6.86699 9.64102 6.58366 9.24935 6.40866 8.92435L6.30866 8.67435C6.25866 8.48268 6.24199 8.37435 6.24199 8.25768C6.24199 7.95768 6.35033 7.69102 6.55866 7.48268L7.18366 6.83268C7.35033 6.66602 7.47533 6.50768 7.55866 6.36602C7.62533 6.25768 7.65033 6.16602 7.65033 6.08268C7.65033 6.01602 7.62533 5.91602 7.58366 5.81602C7.52533 5.68268 7.43366 5.53268 7.31699 5.37435L5.38366 2.64102C5.30033 2.52435 5.20033 2.44102 5.07533 2.38268C4.94199 2.32435 4.80033 2.29102 4.65866 2.29102ZM11.6253 12.5077L11.492 13.0743L11.717 12.491C11.6753 12.4827 11.642 12.491 11.6253 12.5077Z"
                  fill="#292D32"
                />
              </svg>
              <svg
                width="20"
                height="20"
                viewBox="0 0 20 20"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M6.05032 15.7425C5.98365 15.7425 5.917 15.7341 5.85033 15.7258C5.35033 15.6591 4.92533 15.3424 4.717 14.8924L3.66699 12.5841C2.00032 11.3508 1.04199 9.50912 1.04199 7.50911C1.04199 3.95078 4.12533 1.05078 7.91699 1.05078C10.592 1.05078 13.0337 2.51744 14.142 4.79244C14.567 5.63411 14.792 6.55078 14.792 7.50911C14.792 10.9758 11.867 13.8174 8.21699 13.9591L7.167 15.2258C6.88366 15.5508 6.47532 15.7425 6.05032 15.7425ZM7.91699 2.29244C4.81699 2.29244 2.29199 4.62577 2.29199 7.50077C2.29199 9.14244 3.10865 10.6591 4.52532 11.6591C4.61699 11.7258 4.69199 11.8091 4.73365 11.9091L5.85033 14.3674C5.892 14.4591 5.97533 14.4841 6.01699 14.4841C6.05866 14.4924 6.14199 14.4841 6.20033 14.4174L7.43366 12.9341C7.55032 12.7925 7.72533 12.7091 7.91699 12.7091C11.017 12.7091 13.542 10.3758 13.542 7.50077C13.542 6.74244 13.367 6.01742 13.0253 5.34242C12.1253 3.48408 10.117 2.29244 7.91699 2.29244Z"
                  fill="#292D32"
                />
                <path
                  d="M13.9504 18.9576C13.5171 18.9576 13.1171 18.7659 12.8337 18.4326L11.7837 17.1743C9.76706 17.0909 7.86706 16.1659 6.65039 14.6659C6.45872 14.4326 6.46706 14.0993 6.65873 13.8743L7.43372 12.941C7.55039 12.7993 7.72539 12.716 7.91706 12.716C11.0171 12.716 13.5421 10.3826 13.5421 7.50762C13.5421 6.74928 13.3671 6.02426 13.0254 5.34926C12.9171 5.1326 12.9421 4.87427 13.0921 4.68261C13.2421 4.49094 13.4921 4.40759 13.7254 4.45759C16.8087 5.16592 18.9587 7.74094 18.9587 10.7243C18.9587 12.7243 18.0004 14.566 16.3337 15.7993L15.2837 18.1076C15.0671 18.5743 14.6421 18.8909 14.1337 18.9493C14.0754 18.9576 14.0087 18.9576 13.9504 18.9576ZM7.96706 14.2493C9.00872 15.3076 10.5087 15.9243 12.0837 15.9243C12.2671 15.9243 12.4421 16.0076 12.5671 16.1493L13.8004 17.6326C13.8587 17.7076 13.9337 17.7159 13.9837 17.7076C14.0171 17.7076 14.1087 17.6826 14.1587 17.5826L15.2754 15.1243C15.3254 15.0243 15.3921 14.9326 15.4837 14.8743C16.9004 13.8743 17.7171 12.3576 17.7171 10.7159C17.7171 8.71593 16.4921 6.94097 14.6254 6.06597C14.7421 6.53263 14.8004 7.00761 14.8004 7.49928C14.8004 10.9659 11.8754 13.8076 8.22539 13.9493L7.96706 14.2493Z"
                  fill="#292D32"
                />
                <path
                  d="M9.99967 8.125H5.83301C5.49134 8.125 5.20801 7.84167 5.20801 7.5C5.20801 7.15833 5.49134 6.875 5.83301 6.875H9.99967C10.3413 6.875 10.6247 7.15833 10.6247 7.5C10.6247 7.84167 10.3413 8.125 9.99967 8.125Z"
                  fill="#292D32"
                />
              </svg>
              <svg
                width="20"
                height="20"
                viewBox="0 0 20 20"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M2.91667 5.41667L9.39255 10.3688C9.75111 10.643 10.2489 10.643 10.6074 10.3688L17.0833 5.41667M3.5 15H16.5C17.0523 15 17.5 14.5523 17.5 14V6C17.5 5.44772 17.0523 5 16.5 5H3.5C2.94772 5 2.5 5.44772 2.5 6V14C2.5 14.5523 2.94772 15 3.5 15Z"
                  stroke="#0E0E2C"
                  stroke-width="1.5"
                />
              </svg>
            </div>
          </div>
        </div>
        <div className="case-details__inner__grid">
          <div className="case-details__inner__grid__input">
            <label htmlFor="">Court Number</label>
            {/* <input type="text" className="" placeholder="Type court number" /> */}
            <div className="case-details__inner__grid__input__contact">
              Ahmed Aisha
            </div>
          </div>
          <div className="case-details__inner__grid__input">
            <label htmlFor="">Court Location:</label>
            {/* <select name="" id="">
            <option value="">Select court location</option>
          </select> */}
            <div className="case-details__inner__grid__input__filled">
              Federal High Court, Abuja
            </div>
          </div>
          <div className="case-details__inner__grid__input">
            <label htmlFor="">Court Clerk:</label>
            {/* <input type="text" className="" placeholder="Type Court Clerk" /> */}
            <div className="case-details__inner__grid__input__filled">
              Hauwa Jada
            </div>
          </div>

          <div className="case-details__inner__grid__input">
            <label htmlFor="">Court Location:</label>
            {/* <select name="" id="">
            <option value="">Select assigned Judge</option>
          </select> */}
            <div className="case-details__inner__grid__input__contact">
              Olatunde Cardoso
            </div>
          </div>
        </div>
        <div className="case-details__inner__grid">
          <div className="case-details__inner__grid__input">
            <label htmlFor="">Charge Sheet No</label>
            {/* <input type="text" className="" placeholder="Type Sheet No" /> */}
            <div className="case-details__inner__grid__input__contact">
              NPF898458
            </div>
          </div>
          <div className="case-details__inner__grid__input">
            <label htmlFor="">Offence Type:</label>
            {/* <select name="" id="">
            <option value="">Select Offence Type</option>
          </select> */}
            <div className="case-details__inner__grid__input__filled">
              Armed Robbery
            </div>
          </div>
          <div className="case-details__inner__grid__input">
            <label htmlFor="">Agency:</label>
            {/* <select name="" id="">
            <option value="">Select Agency</option>
          </select> */}

            <div className="case-details__inner__grid__input__filled">
              EFCC ABUJA
            </div>
          </div>

          <div className="case-details__inner__grid__input">
            <label htmlFor="">Investigatiing Officer:</label>
            {/* <div className="case-details__inner__grid__input__flex">
            <input type="text" placeholder="Select assigned officer" />
            <svg
              width="21"
              height="20"
              viewBox="0 0 21 20"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <g clip-path="url(#clip0_4643_76588)">
                <path
                  d="M15.4497 14.9489C12.7161 17.6825 8.28392 17.6825 5.55025 14.9489C2.81658 12.2152 2.81658 7.78303 5.55025 5.04936C8.28392 2.31569 12.7161 2.31569 15.4497 5.04936C18.1834 7.78303 18.1834 12.2152 15.4497 14.9489ZM15.4497 14.9489L21 20.4991"
                  stroke="#7E92A2"
                  stroke-width="1.5"
                />
              </g>
              <defs>
                <clipPath id="clip0_4643_76588">
                  <rect
                    width="20"
                    height="20"
                    fill="white"
                    transform="translate(0.5)"
                  />
                </clipPath>
              </defs>
            </svg>
          </div> */}

            <div className="case-details__inner__grid__input__contact">
              Ahmed Aisha
              <svg
                width="20"
                height="20"
                viewBox="0 0 20 20"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M14.542 18.9577C13.6003 18.9577 12.6087 18.7327 11.5837 18.2993C10.5837 17.8743 9.57533 17.291 8.59199 16.5827C7.61699 15.866 6.67533 15.066 5.78366 14.191C4.90033 13.2993 4.10033 12.3577 3.39199 11.391C2.67533 10.391 2.10033 9.39102 1.69199 8.42435C1.25866 7.39102 1.04199 6.39102 1.04199 5.44935C1.04199 4.79935 1.15866 4.18268 1.38366 3.60768C1.61699 3.01602 1.99199 2.46602 2.50033 1.99102C3.14199 1.35768 3.87533 1.04102 4.65866 1.04102C4.98366 1.04102 5.31699 1.11602 5.60033 1.24935C5.92533 1.39935 6.20033 1.62435 6.40033 1.92435L8.33366 4.64935C8.50866 4.89102 8.64199 5.12435 8.73366 5.35768C8.84199 5.60768 8.90033 5.85768 8.90033 6.09935C8.90033 6.41602 8.80866 6.72435 8.63366 7.01602C8.50866 7.24102 8.31699 7.48268 8.07533 7.72435L7.50866 8.31602C7.51699 8.34102 7.52533 8.35768 7.53366 8.37435C7.63366 8.54935 7.83366 8.84935 8.21699 9.29935C8.62533 9.76602 9.00866 10.191 9.39199 10.5827C9.88366 11.066 10.292 11.4493 10.6753 11.766C11.1503 12.166 11.4587 12.366 11.642 12.4577L11.6253 12.4993L12.2337 11.8993C12.492 11.641 12.742 11.4493 12.9837 11.3243C13.442 11.041 14.0253 10.991 14.6087 11.2327C14.8253 11.3243 15.0587 11.4493 15.3087 11.6243L18.0753 13.591C18.3837 13.7993 18.6087 14.066 18.742 14.3827C18.867 14.6993 18.9253 14.991 18.9253 15.2827C18.9253 15.6827 18.8337 16.0827 18.6587 16.4577C18.4837 16.8327 18.267 17.1577 17.992 17.4577C17.517 17.9827 17.0003 18.3577 16.4003 18.5993C15.8253 18.8327 15.2003 18.9577 14.542 18.9577ZM4.65866 2.29102C4.20033 2.29102 3.77533 2.49102 3.36699 2.89102C2.98366 3.24935 2.71699 3.64102 2.55033 4.06602C2.37533 4.49935 2.29199 4.95768 2.29199 5.44935C2.29199 6.22435 2.47533 7.06602 2.84199 7.93268C3.21699 8.81602 3.74199 9.73268 4.40866 10.6493C5.07533 11.566 5.83366 12.4577 6.66699 13.2993C7.50033 14.1243 8.40033 14.891 9.32533 15.566C10.2253 16.2243 11.1503 16.7577 12.067 17.141C13.492 17.7493 14.8253 17.891 15.9253 17.4327C16.3503 17.2577 16.7253 16.991 17.067 16.6077C17.2587 16.3994 17.4087 16.1743 17.5337 15.9077C17.6337 15.6993 17.6837 15.4827 17.6837 15.266C17.6837 15.1327 17.6587 14.9993 17.592 14.8493C17.567 14.7993 17.517 14.7077 17.3587 14.5993L14.592 12.6327C14.4253 12.516 14.2753 12.4327 14.1337 12.3743C13.9503 12.2993 13.8753 12.2243 13.592 12.3993C13.4253 12.4827 13.2753 12.6077 13.1087 12.7743L12.4753 13.3993C12.1503 13.716 11.6503 13.791 11.267 13.6493L11.042 13.5493C10.7003 13.366 10.3003 13.0827 9.85866 12.7077C9.45866 12.366 9.02533 11.966 8.50033 11.4493C8.09199 11.0327 7.68366 10.591 7.25866 10.0993C6.86699 9.64102 6.58366 9.24935 6.40866 8.92435L6.30866 8.67435C6.25866 8.48268 6.24199 8.37435 6.24199 8.25768C6.24199 7.95768 6.35033 7.69102 6.55866 7.48268L7.18366 6.83268C7.35033 6.66602 7.47533 6.50768 7.55866 6.36602C7.62533 6.25768 7.65033 6.16602 7.65033 6.08268C7.65033 6.01602 7.62533 5.91602 7.58366 5.81602C7.52533 5.68268 7.43366 5.53268 7.31699 5.37435L5.38366 2.64102C5.30033 2.52435 5.20033 2.44102 5.07533 2.38268C4.94199 2.32435 4.80033 2.29102 4.65866 2.29102ZM11.6253 12.5077L11.492 13.0743L11.717 12.491C11.6753 12.4827 11.642 12.491 11.6253 12.5077Z"
                  fill="#292D32"
                />
              </svg>
              <svg
                width="20"
                height="20"
                viewBox="0 0 20 20"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M6.05032 15.7425C5.98365 15.7425 5.917 15.7341 5.85033 15.7258C5.35033 15.6591 4.92533 15.3424 4.717 14.8924L3.66699 12.5841C2.00032 11.3508 1.04199 9.50912 1.04199 7.50911C1.04199 3.95078 4.12533 1.05078 7.91699 1.05078C10.592 1.05078 13.0337 2.51744 14.142 4.79244C14.567 5.63411 14.792 6.55078 14.792 7.50911C14.792 10.9758 11.867 13.8174 8.21699 13.9591L7.167 15.2258C6.88366 15.5508 6.47532 15.7425 6.05032 15.7425ZM7.91699 2.29244C4.81699 2.29244 2.29199 4.62577 2.29199 7.50077C2.29199 9.14244 3.10865 10.6591 4.52532 11.6591C4.61699 11.7258 4.69199 11.8091 4.73365 11.9091L5.85033 14.3674C5.892 14.4591 5.97533 14.4841 6.01699 14.4841C6.05866 14.4924 6.14199 14.4841 6.20033 14.4174L7.43366 12.9341C7.55032 12.7925 7.72533 12.7091 7.91699 12.7091C11.017 12.7091 13.542 10.3758 13.542 7.50077C13.542 6.74244 13.367 6.01742 13.0253 5.34242C12.1253 3.48408 10.117 2.29244 7.91699 2.29244Z"
                  fill="#292D32"
                />
                <path
                  d="M13.9504 18.9576C13.5171 18.9576 13.1171 18.7659 12.8337 18.4326L11.7837 17.1743C9.76706 17.0909 7.86706 16.1659 6.65039 14.6659C6.45872 14.4326 6.46706 14.0993 6.65873 13.8743L7.43372 12.941C7.55039 12.7993 7.72539 12.716 7.91706 12.716C11.0171 12.716 13.5421 10.3826 13.5421 7.50762C13.5421 6.74928 13.3671 6.02426 13.0254 5.34926C12.9171 5.1326 12.9421 4.87427 13.0921 4.68261C13.2421 4.49094 13.4921 4.40759 13.7254 4.45759C16.8087 5.16592 18.9587 7.74094 18.9587 10.7243C18.9587 12.7243 18.0004 14.566 16.3337 15.7993L15.2837 18.1076C15.0671 18.5743 14.6421 18.8909 14.1337 18.9493C14.0754 18.9576 14.0087 18.9576 13.9504 18.9576ZM7.96706 14.2493C9.00872 15.3076 10.5087 15.9243 12.0837 15.9243C12.2671 15.9243 12.4421 16.0076 12.5671 16.1493L13.8004 17.6326C13.8587 17.7076 13.9337 17.7159 13.9837 17.7076C14.0171 17.7076 14.1087 17.6826 14.1587 17.5826L15.2754 15.1243C15.3254 15.0243 15.3921 14.9326 15.4837 14.8743C16.9004 13.8743 17.7171 12.3576 17.7171 10.7159C17.7171 8.71593 16.4921 6.94097 14.6254 6.06597C14.7421 6.53263 14.8004 7.00761 14.8004 7.49928C14.8004 10.9659 11.8754 13.8076 8.22539 13.9493L7.96706 14.2493Z"
                  fill="#292D32"
                />
                <path
                  d="M9.99967 8.125H5.83301C5.49134 8.125 5.20801 7.84167 5.20801 7.5C5.20801 7.15833 5.49134 6.875 5.83301 6.875H9.99967C10.3413 6.875 10.6247 7.15833 10.6247 7.5C10.6247 7.84167 10.3413 8.125 9.99967 8.125Z"
                  fill="#292D32"
                />
              </svg>
              <svg
                width="20"
                height="20"
                viewBox="0 0 20 20"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M2.91667 5.41667L9.39255 10.3688C9.75111 10.643 10.2489 10.643 10.6074 10.3688L17.0833 5.41667M3.5 15H16.5C17.0523 15 17.5 14.5523 17.5 14V6C17.5 5.44772 17.0523 5 16.5 5H3.5C2.94772 5 2.5 5.44772 2.5 6V14C2.5 14.5523 2.94772 15 3.5 15Z"
                  stroke="#0E0E2C"
                  stroke-width="1.5"
                />
              </svg>
            </div>
          </div>
        </div>

        <div className="case-details__inner__accused">
          <div className="case-details__inner__accused__bio">
            <div className="case-details__inner__accused__bio__title">
              Accused Bio
            </div>
            <Image
              className="case-details__inner__accused__bio__img"
              alt=""
              src="/assets/Image (2).png"
              width={215}
              height={171}
            />
            <label className="case-details__inner__accused__bio__img-label">
              <svg
                width="17"
                height="17"
                viewBox="0 0 17 17"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M7.89286 12.8435V2.33993L5.06357 5.16921L4.20386 4.29736L8.5 0L12.7974 4.29736L11.9376 5.17043L9.10714 2.33993V12.8435H7.89286ZM1.96229 17C1.4029 17 0.936214 16.813 0.562214 16.439C0.188214 16.065 0.000809524 15.5979 0 15.0377V12.0955H1.21429V15.0377C1.21429 15.2247 1.292 15.3963 1.44743 15.5526C1.60286 15.7088 1.77407 15.7865 1.96107 15.7857H15.0389C15.2251 15.7857 15.3963 15.708 15.5526 15.5526C15.7088 15.3971 15.7865 15.2255 15.7857 15.0377V12.0955H17V15.0377C17 15.5971 16.813 16.0638 16.439 16.4378C16.065 16.8118 15.5979 16.9992 15.0377 17H1.96229Z"
                  fill="#696969"
                />
              </svg>
              Please upload Image
            </label>

            <div className="case-details__inner__accused__bio__input">
              <label htmlFor="">Full Name</label>
              {/* <input
              type="text"
              name=""
              id=""
              placeholder="Type Accused Full Name"
            /> */}
              <div className="case-details__inner__accused__bio__input__filled">
                Frank Ammadou
                <svg
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M9 13V12C9 10.346 10.346 9 12 9H13V5.667C13 5.299 12.701 5 12.333 5H5.667C5.299 5 5 5.299 5 5.667V12.333C5 12.701 5.299 13 5.667 13H9ZM9 15H5.667C4.196 15 3 13.804 3 12.333V5.667C3 4.196 4.196 3 5.667 3H12.333C13.804 3 15 4.196 15 5.667V9H18C19.654 9 21 10.346 21 12V18C21 19.654 19.654 21 18 21H12C10.346 21 9 19.654 9 18V15ZM11 12C11 11.449 11.449 11 12 11H18C18.552 11 19 11.449 19 12V18C19 18.551 18.552 19 18 19H12C11.449 19 11 18.551 11 18V12Z"
                    fill="black"
                  />
                </svg>
              </div>
            </div>
            <div className="case-details__inner__accused__bio__input">
              <label htmlFor="">Alias:</label>
              {/* <input type="text" name="" id="" placeholder="Type Accused Alias" /> */}

              <div className="case-details__inner__accused__bio__input__filled">
                Ororomo
                <svg
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M9 13V12C9 10.346 10.346 9 12 9H13V5.667C13 5.299 12.701 5 12.333 5H5.667C5.299 5 5 5.299 5 5.667V12.333C5 12.701 5.299 13 5.667 13H9ZM9 15H5.667C4.196 15 3 13.804 3 12.333V5.667C3 4.196 4.196 3 5.667 3H12.333C13.804 3 15 4.196 15 5.667V9H18C19.654 9 21 10.346 21 12V18C21 19.654 19.654 21 18 21H12C10.346 21 9 19.654 9 18V15ZM11 12C11 11.449 11.449 11 12 11H18C18.552 11 19 11.449 19 12V18C19 18.551 18.552 19 18 19H12C11.449 19 11 18.551 11 18V12Z"
                    fill="black"
                  />
                </svg>
              </div>
            </div>
            <div className="case-details__inner__accused__bio__input">
              <label htmlFor="">Case Status:</label>

              {/* 
            <select name="" id="">
              <option value="">Select Status</option>
            </select> */}
              <button
                className="case-details__inner__accused__bio__input__case-status"
                disabled
              >
                Filed in Court
              </button>
            </div>
            <div className="case-details__inner__accused__bio__input">
              <label htmlFor="">Accused Status:</label>
              {/* <select name="" id="">
              <option value="">Select</option>
            </select> */}
              <button
                className="case-details__inner__accused__bio__input__case-remanded"
                disabled
              >
                Remamnded in Jail
              </button>
            </div>
            <div className="case-details__inner__accused__bio__input">
              <label htmlFor="">Date Initiated:</label>
              {/* <input
              type="date"
              name=""
              id=""
              placeholder="Type Accused Full Name"
            /> */}

              <button
                className="case-details__inner__accused__bio__input__case-date"
                disabled
              >
                31-12-2023
              </button>
            </div>
            <div className="case-details__inner__accused__bio__input case-details__inner__accused__bio__assoc">
              <label htmlFor="">Known Associates:</label>
              {/* <select name="" id="">
              <option value="">Type Associates</option>
            </select> */}

              {/* <div className="case-details__inner__accused__bio__assoc__list">
              <div>Shekau Ahmed</div>
              <div></div>
            </div> */}

              <div className="case-details__inner__accused__bio__input__case-assoc">
                <div>Okoro Madu</div>
                <div>Shekau Ahmed</div>
                <div>Tope Adeniyi</div>
              </div>
            </div>
          </div>
          <div className="case-details__inner__accused__details">
            <div className="case-details__inner__accused__details__input">
              <label htmlFor="">Legal Basis</label>
              <textarea
                name=""
                id=""
                placeholder="Insert the legal basis"
                disabled
                value="The accused, on April 15, 2024, at approximately 11:30 AM, at the XYZ Bank branch located on Victoria Island, Lagos, Nigeria, did unlawfully and intentionally commit armed robbery. The accused, armed with pistols and wearing masks, entered the bank premises and threatened customers and staff with violence. The accused forcibly stole a significant amount of cash and valuables from the bank, thereby committing the offense of armed robbery."
              ></textarea>
              <div className="case-details__inner__accused__details__input__read">
                Read More
              </div>
            </div>
            <div className="case-details__inner__accused__details__input">
              <label htmlFor="">Particulars of Offence</label>
              <textarea
                name=""
                id=""
                placeholder="Insert the particulars of offence"
                disabled
                value="The accused, on April 15, 2024, at approximately 11:30 AM, at the XYZ Bank branch located on Victoria Island, Lagos, Nigeria, did unlawfully and intentionally commit armed robbery. The accused, armed with pistols and wearing masks, entered the bank premises and threatened customers and staff with violence. The accused forcibly stole a significant amount of cash and valuables from the bank, thereby committing the offense of armed robbery."
              ></textarea>
              <div className="case-details__inner__accused__details__input__read">
                Read More
              </div>
            </div>
            <div className="case-details__inner__accused__details__input">
              <label htmlFor="">Charge Details</label>
              <textarea
                name=""
                id=""
                placeholder="Insert the charge details"
                value="The accused, on April 15, 2024, at approximately 11:30 AM, at the XYZ Bank branch located on Victoria Island, Lagos, Nigeria, did unlawfully and intentionally commit armed robbery. The accused, armed with pistols and wearing masks, entered the bank premises and threatened customers and staff with violence. The accused forcibly stole a significant amount of cash and valuables from the bank, thereby committing the offense of armed robbery."
                disabled
              ></textarea>
              <div className="case-details__inner__accused__details__input__read">
                Read More
              </div>
            </div>
            <div className="case-details__inner__accused__details__input">
              <label htmlFor="">Legal Brief and Memoranda</label>
              <textarea
                name=""
                id=""
                placeholder="Insert the legal brief and memoranda"
                disabled
                value="The accused, on April 15, 2024, at approximately 11:30 AM, at the XYZ Bank branch located on Victoria Island, Lagos, Nigeria, did unlawfully and intentionally commit armed robbery. The accused, armed with pistols and wearing masks, entered the bank premises and threatened customers and staff with violence. The accused forcibly stole a significant amount of cash and valuables from the bank, thereby committing the offense of armed robbery."
              ></textarea>
              <div className="case-details__inner__accused__details__input__read">
                Read More
              </div>
            </div>
          </div>
          <div className="case-details__inner__accused__attachment">
            <div className="case-details__inner__accused__attachment__title">
              ATTACHMENT
            </div>
            <div className="case-details__inner__accused__attachment__doc case-details__inner__accused__attachment__doc-first">
              Attach New Document{" "}
              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M18 12.75H6C5.59 12.75 5.25 12.41 5.25 12C5.25 11.59 5.59 11.25 6 11.25H18C18.41 11.25 18.75 11.59 18.75 12C18.75 12.41 18.41 12.75 18 12.75Z"
                  fill="#292D32"
                />
                <path
                  d="M12 18.75C11.59 18.75 11.25 18.41 11.25 18V6C11.25 5.59 11.59 5.25 12 5.25C12.41 5.25 12.75 5.59 12.75 6V18C12.75 18.41 12.41 18.75 12 18.75Z"
                  fill="#292D32"
                />
              </svg>
            </div>
            <div className="case-details__inner__accused__attachment__doc case-details__inner__accused__attachment__doc-filled">
              {/* Attach Charge sheet */}
              <div>
                <svg
                  width="20"
                  height="21"
                  viewBox="0 0 20 21"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M15.4417 8.00065L10.8333 3.39232C10.7162 3.27507 10.5574 3.20913 10.3917 3.20898H6.66667C6.05888 3.20898 5.47598 3.45043 5.04621 3.8802C4.61644 4.30997 4.375 4.89286 4.375 5.50065V15.5007C4.375 16.1084 4.61644 16.6913 5.04621 17.1211C5.47598 17.5509 6.05888 17.7923 6.66667 17.7923H13.3333C13.9411 17.7923 14.524 17.5509 14.9538 17.1211C15.3836 16.6913 15.625 16.1084 15.625 15.5007V8.41732C15.6184 8.26032 15.553 8.11156 15.4417 8.00065ZM11.0417 5.34232L13.4917 7.79232H11.0417V5.34232ZM13.3333 16.5423H6.66667C6.3904 16.5423 6.12545 16.4326 5.9301 16.2372C5.73475 16.0419 5.625 15.7769 5.625 15.5007V5.50065C5.625 5.22438 5.73475 4.95943 5.9301 4.76408C6.12545 4.56873 6.3904 4.45898 6.66667 4.45898H9.79167V8.41732C9.79382 8.58241 9.86037 8.74013 9.97711 8.85687C10.0939 8.97362 10.2516 9.04016 10.4167 9.04232H14.375V15.5007C14.375 15.7769 14.2653 16.0419 14.0699 16.2372C13.8746 16.4326 13.6096 16.5423 13.3333 16.5423Z"
                    fill="#334655"
                  />
                  <path
                    d="M11.2417 12.8743C10.7293 12.5528 10.3436 12.0641 10.15 11.491C10.3294 10.9548 10.3836 10.3847 10.3083 9.82433C10.2843 9.68316 10.2155 9.55347 10.112 9.45448C10.0085 9.35549 9.87592 9.29244 9.73382 9.27468C9.59172 9.25692 9.44769 9.28538 9.32303 9.35586C9.19837 9.42634 9.09972 9.53508 9.04168 9.66599C8.9468 10.3397 9.01833 11.0263 9.25001 11.666C8.93351 12.4054 8.58029 13.1285 8.19168 13.8327C7.60001 14.166 6.79168 14.666 6.66668 15.241C6.56668 15.7077 7.44168 16.9077 8.93334 14.3077C9.59662 14.0614 10.276 13.861 10.9667 13.7077C11.4772 13.9996 12.0468 14.1733 12.6333 14.216C12.768 14.2195 12.9007 14.1831 13.0148 14.1114C13.1288 14.0397 13.2192 13.9358 13.2744 13.8129C13.3296 13.69 13.3472 13.5536 13.3251 13.4207C13.303 13.2878 13.2421 13.1644 13.15 13.066C12.8 12.7077 11.7583 12.8077 11.2417 12.8743ZM7.25834 15.3743C7.49188 14.9747 7.80028 14.6238 8.16668 14.341C7.60001 15.241 7.25834 15.3993 7.25834 15.3827V15.3743ZM9.69168 9.69933C9.90834 9.69933 9.89168 10.6577 9.74168 10.916C9.62868 10.523 9.61154 10.1086 9.69168 9.70766V9.69933ZM8.96668 13.766C9.24902 13.2509 9.49409 12.7162 9.70001 12.166C9.92077 12.5768 10.2278 12.935 10.6 13.216C10.0409 13.356 9.49453 13.5428 8.96668 13.7743V13.766ZM12.8833 13.616C12.8833 13.616 12.7333 13.7993 11.775 13.3827C12.8167 13.316 12.9917 13.5577 12.8833 13.6243V13.616Z"
                    fill="#334655"
                  />
                </svg>
                Charge_sheet.PDF
              </div>
              {/* <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M18 12.75H6C5.59 12.75 5.25 12.41 5.25 12C5.25 11.59 5.59 11.25 6 11.25H18C18.41 11.25 18.75 11.59 18.75 12C18.75 12.41 18.41 12.75 18 12.75Z"
                fill="#292D32"
              />
              <path
                d="M12 18.75C11.59 18.75 11.25 18.41 11.25 18V6C11.25 5.59 11.59 5.25 12 5.25C12.41 5.25 12.75 5.59 12.75 6V18C12.75 18.41 12.41 18.75 12 18.75Z"
                fill="#292D32"
              />
            </svg> */}

              {/* <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M12.0001 13.0597L6.53039 18.5294L5.46973 17.4688L10.9394 11.9991L5.46973 6.52941L6.53039 5.46875L12.0001 10.9384L17.4697 5.46875L18.5304 6.52941L13.0607 11.9991L18.5304 17.4688L17.4697 18.5294L12.0001 13.0597Z" fill="#0E0E2C"/>
</svg> */}

              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M12.0001 21.2491C11.8101 21.2491 11.6201 21.1791 11.4701 21.0291L5.40012 14.9591C5.11012 14.6691 5.11012 14.1891 5.40012 13.8991C5.69012 13.6091 6.17012 13.6091 6.46012 13.8991L12.0001 19.4391L17.5401 13.8991C17.8301 13.6091 18.3101 13.6091 18.6001 13.8991C18.8901 14.1891 18.8901 14.6691 18.6001 14.9591L12.5301 21.0291C12.3801 21.1791 12.1901 21.2491 12.0001 21.2491Z"
                  fill="#292D32"
                />
                <path
                  d="M12 21.08C11.59 21.08 11.25 20.74 11.25 20.33V3.5C11.25 3.09 11.59 2.75 12 2.75C12.41 2.75 12.75 3.09 12.75 3.5V20.33C12.75 20.74 12.41 21.08 12 21.08Z"
                  fill="#292D32"
                />
              </svg>
            </div>
            <div className="case-details__inner__accused__attachment__doc case-details__inner__accused__attachment__doc-filled">
              {/* Attach Evidence and Witness{" "} */}

              <div>
                <svg
                  width="20"
                  height="21"
                  viewBox="0 0 20 21"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M15.4417 8.00065L10.8333 3.39232C10.7162 3.27507 10.5574 3.20913 10.3917 3.20898H6.66667C6.05888 3.20898 5.47598 3.45043 5.04621 3.8802C4.61644 4.30997 4.375 4.89286 4.375 5.50065V15.5007C4.375 16.1084 4.61644 16.6913 5.04621 17.1211C5.47598 17.5509 6.05888 17.7923 6.66667 17.7923H13.3333C13.9411 17.7923 14.524 17.5509 14.9538 17.1211C15.3836 16.6913 15.625 16.1084 15.625 15.5007V8.41732C15.6184 8.26032 15.553 8.11156 15.4417 8.00065ZM11.0417 5.34232L13.4917 7.79232H11.0417V5.34232ZM13.3333 16.5423H6.66667C6.3904 16.5423 6.12545 16.4326 5.9301 16.2372C5.73475 16.0419 5.625 15.7769 5.625 15.5007V5.50065C5.625 5.22438 5.73475 4.95943 5.9301 4.76408C6.12545 4.56873 6.3904 4.45898 6.66667 4.45898H9.79167V8.41732C9.79382 8.58241 9.86037 8.74013 9.97711 8.85687C10.0939 8.97362 10.2516 9.04016 10.4167 9.04232H14.375V15.5007C14.375 15.7769 14.2653 16.0419 14.0699 16.2372C13.8746 16.4326 13.6096 16.5423 13.3333 16.5423Z"
                    fill="#334655"
                  />
                  <path
                    d="M11.2417 12.8743C10.7293 12.5528 10.3436 12.0641 10.15 11.491C10.3294 10.9548 10.3836 10.3847 10.3083 9.82433C10.2843 9.68316 10.2155 9.55347 10.112 9.45448C10.0085 9.35549 9.87592 9.29244 9.73382 9.27468C9.59172 9.25692 9.44769 9.28538 9.32303 9.35586C9.19837 9.42634 9.09972 9.53508 9.04168 9.66599C8.9468 10.3397 9.01833 11.0263 9.25001 11.666C8.93351 12.4054 8.58029 13.1285 8.19168 13.8327C7.60001 14.166 6.79168 14.666 6.66668 15.241C6.56668 15.7077 7.44168 16.9077 8.93334 14.3077C9.59662 14.0614 10.276 13.861 10.9667 13.7077C11.4772 13.9996 12.0468 14.1733 12.6333 14.216C12.768 14.2195 12.9007 14.1831 13.0148 14.1114C13.1288 14.0397 13.2192 13.9358 13.2744 13.8129C13.3296 13.69 13.3472 13.5536 13.3251 13.4207C13.303 13.2878 13.2421 13.1644 13.15 13.066C12.8 12.7077 11.7583 12.8077 11.2417 12.8743ZM7.25834 15.3743C7.49188 14.9747 7.80028 14.6238 8.16668 14.341C7.60001 15.241 7.25834 15.3993 7.25834 15.3827V15.3743ZM9.69168 9.69933C9.90834 9.69933 9.89168 10.6577 9.74168 10.916C9.62868 10.523 9.61154 10.1086 9.69168 9.70766V9.69933ZM8.96668 13.766C9.24902 13.2509 9.49409 12.7162 9.70001 12.166C9.92077 12.5768 10.2278 12.935 10.6 13.216C10.0409 13.356 9.49453 13.5428 8.96668 13.7743V13.766ZM12.8833 13.616C12.8833 13.616 12.7333 13.7993 11.775 13.3827C12.8167 13.316 12.9917 13.5577 12.8833 13.6243V13.616Z"
                    fill="#334655"
                  />
                </svg>
                Evidence and Witness.PDF
              </div>
              {/* <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M18 12.75H6C5.59 12.75 5.25 12.41 5.25 12C5.25 11.59 5.59 11.25 6 11.25H18C18.41 11.25 18.75 11.59 18.75 12C18.75 12.41 18.41 12.75 18 12.75Z"
                fill="#292D32"
              />
              <path
                d="M12 18.75C11.59 18.75 11.25 18.41 11.25 18V6C11.25 5.59 11.59 5.25 12 5.25C12.41 5.25 12.75 5.59 12.75 6V18C12.75 18.41 12.41 18.75 12 18.75Z"
                fill="#292D32"
              />
            </svg> */}

              {/* <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M12.0001 13.0597L6.53039 18.5294L5.46973 17.4688L10.9394 11.9991L5.46973 6.52941L6.53039 5.46875L12.0001 10.9384L17.4697 5.46875L18.5304 6.52941L13.0607 11.9991L18.5304 17.4688L17.4697 18.5294L12.0001 13.0597Z" fill="#0E0E2C"/>
</svg> */}

              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M12.0001 21.2491C11.8101 21.2491 11.6201 21.1791 11.4701 21.0291L5.40012 14.9591C5.11012 14.6691 5.11012 14.1891 5.40012 13.8991C5.69012 13.6091 6.17012 13.6091 6.46012 13.8991L12.0001 19.4391L17.5401 13.8991C17.8301 13.6091 18.3101 13.6091 18.6001 13.8991C18.8901 14.1891 18.8901 14.6691 18.6001 14.9591L12.5301 21.0291C12.3801 21.1791 12.1901 21.2491 12.0001 21.2491Z"
                  fill="#292D32"
                />
                <path
                  d="M12 21.08C11.59 21.08 11.25 20.74 11.25 20.33V3.5C11.25 3.09 11.59 2.75 12 2.75C12.41 2.75 12.75 3.09 12.75 3.5V20.33C12.75 20.74 12.41 21.08 12 21.08Z"
                  fill="#292D32"
                />
              </svg>
            </div>
            <div className="case-details__inner__accused__attachment__doc case-details__inner__accused__attachment__doc-filled">
              {/* Attach Expert Report{" "} */}

              <div>
                <svg
                  width="20"
                  height="21"
                  viewBox="0 0 20 21"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M15.4417 8.00065L10.8333 3.39232C10.7162 3.27507 10.5574 3.20913 10.3917 3.20898H6.66667C6.05888 3.20898 5.47598 3.45043 5.04621 3.8802C4.61644 4.30997 4.375 4.89286 4.375 5.50065V15.5007C4.375 16.1084 4.61644 16.6913 5.04621 17.1211C5.47598 17.5509 6.05888 17.7923 6.66667 17.7923H13.3333C13.9411 17.7923 14.524 17.5509 14.9538 17.1211C15.3836 16.6913 15.625 16.1084 15.625 15.5007V8.41732C15.6184 8.26032 15.553 8.11156 15.4417 8.00065ZM11.0417 5.34232L13.4917 7.79232H11.0417V5.34232ZM13.3333 16.5423H6.66667C6.3904 16.5423 6.12545 16.4326 5.9301 16.2372C5.73475 16.0419 5.625 15.7769 5.625 15.5007V5.50065C5.625 5.22438 5.73475 4.95943 5.9301 4.76408C6.12545 4.56873 6.3904 4.45898 6.66667 4.45898H9.79167V8.41732C9.79382 8.58241 9.86037 8.74013 9.97711 8.85687C10.0939 8.97362 10.2516 9.04016 10.4167 9.04232H14.375V15.5007C14.375 15.7769 14.2653 16.0419 14.0699 16.2372C13.8746 16.4326 13.6096 16.5423 13.3333 16.5423Z"
                    fill="#334655"
                  />
                  <path
                    d="M11.2417 12.8743C10.7293 12.5528 10.3436 12.0641 10.15 11.491C10.3294 10.9548 10.3836 10.3847 10.3083 9.82433C10.2843 9.68316 10.2155 9.55347 10.112 9.45448C10.0085 9.35549 9.87592 9.29244 9.73382 9.27468C9.59172 9.25692 9.44769 9.28538 9.32303 9.35586C9.19837 9.42634 9.09972 9.53508 9.04168 9.66599C8.9468 10.3397 9.01833 11.0263 9.25001 11.666C8.93351 12.4054 8.58029 13.1285 8.19168 13.8327C7.60001 14.166 6.79168 14.666 6.66668 15.241C6.56668 15.7077 7.44168 16.9077 8.93334 14.3077C9.59662 14.0614 10.276 13.861 10.9667 13.7077C11.4772 13.9996 12.0468 14.1733 12.6333 14.216C12.768 14.2195 12.9007 14.1831 13.0148 14.1114C13.1288 14.0397 13.2192 13.9358 13.2744 13.8129C13.3296 13.69 13.3472 13.5536 13.3251 13.4207C13.303 13.2878 13.2421 13.1644 13.15 13.066C12.8 12.7077 11.7583 12.8077 11.2417 12.8743ZM7.25834 15.3743C7.49188 14.9747 7.80028 14.6238 8.16668 14.341C7.60001 15.241 7.25834 15.3993 7.25834 15.3827V15.3743ZM9.69168 9.69933C9.90834 9.69933 9.89168 10.6577 9.74168 10.916C9.62868 10.523 9.61154 10.1086 9.69168 9.70766V9.69933ZM8.96668 13.766C9.24902 13.2509 9.49409 12.7162 9.70001 12.166C9.92077 12.5768 10.2278 12.935 10.6 13.216C10.0409 13.356 9.49453 13.5428 8.96668 13.7743V13.766ZM12.8833 13.616C12.8833 13.616 12.7333 13.7993 11.775 13.3827C12.8167 13.316 12.9917 13.5577 12.8833 13.6243V13.616Z"
                    fill="#334655"
                  />
                </svg>
                Expert Report.PDF
              </div>
              {/* <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M18 12.75H6C5.59 12.75 5.25 12.41 5.25 12C5.25 11.59 5.59 11.25 6 11.25H18C18.41 11.25 18.75 11.59 18.75 12C18.75 12.41 18.41 12.75 18 12.75Z"
                fill="#292D32"
              />
              <path
                d="M12 18.75C11.59 18.75 11.25 18.41 11.25 18V6C11.25 5.59 11.59 5.25 12 5.25C12.41 5.25 12.75 5.59 12.75 6V18C12.75 18.41 12.41 18.75 12 18.75Z"
                fill="#292D32"
              />
            </svg> */}

              {/* <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M12.0001 13.0597L6.53039 18.5294L5.46973 17.4688L10.9394 11.9991L5.46973 6.52941L6.53039 5.46875L12.0001 10.9384L17.4697 5.46875L18.5304 6.52941L13.0607 11.9991L18.5304 17.4688L17.4697 18.5294L12.0001 13.0597Z" fill="#0E0E2C"/>
</svg> */}

              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M12.0001 21.2491C11.8101 21.2491 11.6201 21.1791 11.4701 21.0291L5.40012 14.9591C5.11012 14.6691 5.11012 14.1891 5.40012 13.8991C5.69012 13.6091 6.17012 13.6091 6.46012 13.8991L12.0001 19.4391L17.5401 13.8991C17.8301 13.6091 18.3101 13.6091 18.6001 13.8991C18.8901 14.1891 18.8901 14.6691 18.6001 14.9591L12.5301 21.0291C12.3801 21.1791 12.1901 21.2491 12.0001 21.2491Z"
                  fill="#292D32"
                />
                <path
                  d="M12 21.08C11.59 21.08 11.25 20.74 11.25 20.33V3.5C11.25 3.09 11.59 2.75 12 2.75C12.41 2.75 12.75 3.09 12.75 3.5V20.33C12.75 20.74 12.41 21.08 12 21.08Z"
                  fill="#292D32"
                />
              </svg>
            </div>
            <div className="case-details__inner__accused__attachment__doc case-details__inner__accused__attachment__doc-filled">
              {/* Attach Court Document{" "} */}

              <div>
                <svg
                  width="20"
                  height="21"
                  viewBox="0 0 20 21"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M15.4417 8.00065L10.8333 3.39232C10.7162 3.27507 10.5574 3.20913 10.3917 3.20898H6.66667C6.05888 3.20898 5.47598 3.45043 5.04621 3.8802C4.61644 4.30997 4.375 4.89286 4.375 5.50065V15.5007C4.375 16.1084 4.61644 16.6913 5.04621 17.1211C5.47598 17.5509 6.05888 17.7923 6.66667 17.7923H13.3333C13.9411 17.7923 14.524 17.5509 14.9538 17.1211C15.3836 16.6913 15.625 16.1084 15.625 15.5007V8.41732C15.6184 8.26032 15.553 8.11156 15.4417 8.00065ZM11.0417 5.34232L13.4917 7.79232H11.0417V5.34232ZM13.3333 16.5423H6.66667C6.3904 16.5423 6.12545 16.4326 5.9301 16.2372C5.73475 16.0419 5.625 15.7769 5.625 15.5007V5.50065C5.625 5.22438 5.73475 4.95943 5.9301 4.76408C6.12545 4.56873 6.3904 4.45898 6.66667 4.45898H9.79167V8.41732C9.79382 8.58241 9.86037 8.74013 9.97711 8.85687C10.0939 8.97362 10.2516 9.04016 10.4167 9.04232H14.375V15.5007C14.375 15.7769 14.2653 16.0419 14.0699 16.2372C13.8746 16.4326 13.6096 16.5423 13.3333 16.5423Z"
                    fill="#334655"
                  />
                  <path
                    d="M11.2417 12.8743C10.7293 12.5528 10.3436 12.0641 10.15 11.491C10.3294 10.9548 10.3836 10.3847 10.3083 9.82433C10.2843 9.68316 10.2155 9.55347 10.112 9.45448C10.0085 9.35549 9.87592 9.29244 9.73382 9.27468C9.59172 9.25692 9.44769 9.28538 9.32303 9.35586C9.19837 9.42634 9.09972 9.53508 9.04168 9.66599C8.9468 10.3397 9.01833 11.0263 9.25001 11.666C8.93351 12.4054 8.58029 13.1285 8.19168 13.8327C7.60001 14.166 6.79168 14.666 6.66668 15.241C6.56668 15.7077 7.44168 16.9077 8.93334 14.3077C9.59662 14.0614 10.276 13.861 10.9667 13.7077C11.4772 13.9996 12.0468 14.1733 12.6333 14.216C12.768 14.2195 12.9007 14.1831 13.0148 14.1114C13.1288 14.0397 13.2192 13.9358 13.2744 13.8129C13.3296 13.69 13.3472 13.5536 13.3251 13.4207C13.303 13.2878 13.2421 13.1644 13.15 13.066C12.8 12.7077 11.7583 12.8077 11.2417 12.8743ZM7.25834 15.3743C7.49188 14.9747 7.80028 14.6238 8.16668 14.341C7.60001 15.241 7.25834 15.3993 7.25834 15.3827V15.3743ZM9.69168 9.69933C9.90834 9.69933 9.89168 10.6577 9.74168 10.916C9.62868 10.523 9.61154 10.1086 9.69168 9.70766V9.69933ZM8.96668 13.766C9.24902 13.2509 9.49409 12.7162 9.70001 12.166C9.92077 12.5768 10.2278 12.935 10.6 13.216C10.0409 13.356 9.49453 13.5428 8.96668 13.7743V13.766ZM12.8833 13.616C12.8833 13.616 12.7333 13.7993 11.775 13.3827C12.8167 13.316 12.9917 13.5577 12.8833 13.6243V13.616Z"
                    fill="#334655"
                  />
                </svg>
                Court Document.PDF
              </div>
              {/* <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M18 12.75H6C5.59 12.75 5.25 12.41 5.25 12C5.25 11.59 5.59 11.25 6 11.25H18C18.41 11.25 18.75 11.59 18.75 12C18.75 12.41 18.41 12.75 18 12.75Z"
                fill="#292D32"
              />
              <path
                d="M12 18.75C11.59 18.75 11.25 18.41 11.25 18V6C11.25 5.59 11.59 5.25 12 5.25C12.41 5.25 12.75 5.59 12.75 6V18C12.75 18.41 12.41 18.75 12 18.75Z"
                fill="#292D32"
              />
            </svg> */}

              {/* <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M12.0001 13.0597L6.53039 18.5294L5.46973 17.4688L10.9394 11.9991L5.46973 6.52941L6.53039 5.46875L12.0001 10.9384L17.4697 5.46875L18.5304 6.52941L13.0607 11.9991L18.5304 17.4688L17.4697 18.5294L12.0001 13.0597Z" fill="#0E0E2C"/>
</svg> */}

              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M12.0001 21.2491C11.8101 21.2491 11.6201 21.1791 11.4701 21.0291L5.40012 14.9591C5.11012 14.6691 5.11012 14.1891 5.40012 13.8991C5.69012 13.6091 6.17012 13.6091 6.46012 13.8991L12.0001 19.4391L17.5401 13.8991C17.8301 13.6091 18.3101 13.6091 18.6001 13.8991C18.8901 14.1891 18.8901 14.6691 18.6001 14.9591L12.5301 21.0291C12.3801 21.1791 12.1901 21.2491 12.0001 21.2491Z"
                  fill="#292D32"
                />
                <path
                  d="M12 21.08C11.59 21.08 11.25 20.74 11.25 20.33V3.5C11.25 3.09 11.59 2.75 12 2.75C12.41 2.75 12.75 3.09 12.75 3.5V20.33C12.75 20.74 12.41 21.08 12 21.08Z"
                  fill="#292D32"
                />
              </svg>
            </div>
            <div className="case-details__inner__accused__attachment__doc case-details__inner__accused__attachment__doc-filled">
              {/* Attach Transcripts{" "} */}

              <div>
                <svg
                  width="20"
                  height="21"
                  viewBox="0 0 20 21"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M15.4417 8.00065L10.8333 3.39232C10.7162 3.27507 10.5574 3.20913 10.3917 3.20898H6.66667C6.05888 3.20898 5.47598 3.45043 5.04621 3.8802C4.61644 4.30997 4.375 4.89286 4.375 5.50065V15.5007C4.375 16.1084 4.61644 16.6913 5.04621 17.1211C5.47598 17.5509 6.05888 17.7923 6.66667 17.7923H13.3333C13.9411 17.7923 14.524 17.5509 14.9538 17.1211C15.3836 16.6913 15.625 16.1084 15.625 15.5007V8.41732C15.6184 8.26032 15.553 8.11156 15.4417 8.00065ZM11.0417 5.34232L13.4917 7.79232H11.0417V5.34232ZM13.3333 16.5423H6.66667C6.3904 16.5423 6.12545 16.4326 5.9301 16.2372C5.73475 16.0419 5.625 15.7769 5.625 15.5007V5.50065C5.625 5.22438 5.73475 4.95943 5.9301 4.76408C6.12545 4.56873 6.3904 4.45898 6.66667 4.45898H9.79167V8.41732C9.79382 8.58241 9.86037 8.74013 9.97711 8.85687C10.0939 8.97362 10.2516 9.04016 10.4167 9.04232H14.375V15.5007C14.375 15.7769 14.2653 16.0419 14.0699 16.2372C13.8746 16.4326 13.6096 16.5423 13.3333 16.5423Z"
                    fill="#334655"
                  />
                  <path
                    d="M11.2417 12.8743C10.7293 12.5528 10.3436 12.0641 10.15 11.491C10.3294 10.9548 10.3836 10.3847 10.3083 9.82433C10.2843 9.68316 10.2155 9.55347 10.112 9.45448C10.0085 9.35549 9.87592 9.29244 9.73382 9.27468C9.59172 9.25692 9.44769 9.28538 9.32303 9.35586C9.19837 9.42634 9.09972 9.53508 9.04168 9.66599C8.9468 10.3397 9.01833 11.0263 9.25001 11.666C8.93351 12.4054 8.58029 13.1285 8.19168 13.8327C7.60001 14.166 6.79168 14.666 6.66668 15.241C6.56668 15.7077 7.44168 16.9077 8.93334 14.3077C9.59662 14.0614 10.276 13.861 10.9667 13.7077C11.4772 13.9996 12.0468 14.1733 12.6333 14.216C12.768 14.2195 12.9007 14.1831 13.0148 14.1114C13.1288 14.0397 13.2192 13.9358 13.2744 13.8129C13.3296 13.69 13.3472 13.5536 13.3251 13.4207C13.303 13.2878 13.2421 13.1644 13.15 13.066C12.8 12.7077 11.7583 12.8077 11.2417 12.8743ZM7.25834 15.3743C7.49188 14.9747 7.80028 14.6238 8.16668 14.341C7.60001 15.241 7.25834 15.3993 7.25834 15.3827V15.3743ZM9.69168 9.69933C9.90834 9.69933 9.89168 10.6577 9.74168 10.916C9.62868 10.523 9.61154 10.1086 9.69168 9.70766V9.69933ZM8.96668 13.766C9.24902 13.2509 9.49409 12.7162 9.70001 12.166C9.92077 12.5768 10.2278 12.935 10.6 13.216C10.0409 13.356 9.49453 13.5428 8.96668 13.7743V13.766ZM12.8833 13.616C12.8833 13.616 12.7333 13.7993 11.775 13.3827C12.8167 13.316 12.9917 13.5577 12.8833 13.6243V13.616Z"
                    fill="#334655"
                  />
                </svg>
                Transcripts.PDF
              </div>
              {/* <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M18 12.75H6C5.59 12.75 5.25 12.41 5.25 12C5.25 11.59 5.59 11.25 6 11.25H18C18.41 11.25 18.75 11.59 18.75 12C18.75 12.41 18.41 12.75 18 12.75Z"
                fill="#292D32"
              />
              <path
                d="M12 18.75C11.59 18.75 11.25 18.41 11.25 18V6C11.25 5.59 11.59 5.25 12 5.25C12.41 5.25 12.75 5.59 12.75 6V18C12.75 18.41 12.41 18.75 12 18.75Z"
                fill="#292D32"
              />
            </svg> */}

              {/* <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M12.0001 13.0597L6.53039 18.5294L5.46973 17.4688L10.9394 11.9991L5.46973 6.52941L6.53039 5.46875L12.0001 10.9384L17.4697 5.46875L18.5304 6.52941L13.0607 11.9991L18.5304 17.4688L17.4697 18.5294L12.0001 13.0597Z" fill="#0E0E2C"/>
</svg> */}

              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M12.0001 21.2491C11.8101 21.2491 11.6201 21.1791 11.4701 21.0291L5.40012 14.9591C5.11012 14.6691 5.11012 14.1891 5.40012 13.8991C5.69012 13.6091 6.17012 13.6091 6.46012 13.8991L12.0001 19.4391L17.5401 13.8991C17.8301 13.6091 18.3101 13.6091 18.6001 13.8991C18.8901 14.1891 18.8901 14.6691 18.6001 14.9591L12.5301 21.0291C12.3801 21.1791 12.1901 21.2491 12.0001 21.2491Z"
                  fill="#292D32"
                />
                <path
                  d="M12 21.08C11.59 21.08 11.25 20.74 11.25 20.33V3.5C11.25 3.09 11.59 2.75 12 2.75C12.41 2.75 12.75 3.09 12.75 3.5V20.33C12.75 20.74 12.41 21.08 12 21.08Z"
                  fill="#292D32"
                />
              </svg>
            </div>
            <div className="case-details__inner__accused__attachment__doc case-details__inner__accused__attachment__doc-filled">
              {/* Attach Discovery Materials{" "} */}

              <div>
                <svg
                  width="20"
                  height="21"
                  viewBox="0 0 20 21"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M15.4417 8.00065L10.8333 3.39232C10.7162 3.27507 10.5574 3.20913 10.3917 3.20898H6.66667C6.05888 3.20898 5.47598 3.45043 5.04621 3.8802C4.61644 4.30997 4.375 4.89286 4.375 5.50065V15.5007C4.375 16.1084 4.61644 16.6913 5.04621 17.1211C5.47598 17.5509 6.05888 17.7923 6.66667 17.7923H13.3333C13.9411 17.7923 14.524 17.5509 14.9538 17.1211C15.3836 16.6913 15.625 16.1084 15.625 15.5007V8.41732C15.6184 8.26032 15.553 8.11156 15.4417 8.00065ZM11.0417 5.34232L13.4917 7.79232H11.0417V5.34232ZM13.3333 16.5423H6.66667C6.3904 16.5423 6.12545 16.4326 5.9301 16.2372C5.73475 16.0419 5.625 15.7769 5.625 15.5007V5.50065C5.625 5.22438 5.73475 4.95943 5.9301 4.76408C6.12545 4.56873 6.3904 4.45898 6.66667 4.45898H9.79167V8.41732C9.79382 8.58241 9.86037 8.74013 9.97711 8.85687C10.0939 8.97362 10.2516 9.04016 10.4167 9.04232H14.375V15.5007C14.375 15.7769 14.2653 16.0419 14.0699 16.2372C13.8746 16.4326 13.6096 16.5423 13.3333 16.5423Z"
                    fill="#334655"
                  />
                  <path
                    d="M11.2417 12.8743C10.7293 12.5528 10.3436 12.0641 10.15 11.491C10.3294 10.9548 10.3836 10.3847 10.3083 9.82433C10.2843 9.68316 10.2155 9.55347 10.112 9.45448C10.0085 9.35549 9.87592 9.29244 9.73382 9.27468C9.59172 9.25692 9.44769 9.28538 9.32303 9.35586C9.19837 9.42634 9.09972 9.53508 9.04168 9.66599C8.9468 10.3397 9.01833 11.0263 9.25001 11.666C8.93351 12.4054 8.58029 13.1285 8.19168 13.8327C7.60001 14.166 6.79168 14.666 6.66668 15.241C6.56668 15.7077 7.44168 16.9077 8.93334 14.3077C9.59662 14.0614 10.276 13.861 10.9667 13.7077C11.4772 13.9996 12.0468 14.1733 12.6333 14.216C12.768 14.2195 12.9007 14.1831 13.0148 14.1114C13.1288 14.0397 13.2192 13.9358 13.2744 13.8129C13.3296 13.69 13.3472 13.5536 13.3251 13.4207C13.303 13.2878 13.2421 13.1644 13.15 13.066C12.8 12.7077 11.7583 12.8077 11.2417 12.8743ZM7.25834 15.3743C7.49188 14.9747 7.80028 14.6238 8.16668 14.341C7.60001 15.241 7.25834 15.3993 7.25834 15.3827V15.3743ZM9.69168 9.69933C9.90834 9.69933 9.89168 10.6577 9.74168 10.916C9.62868 10.523 9.61154 10.1086 9.69168 9.70766V9.69933ZM8.96668 13.766C9.24902 13.2509 9.49409 12.7162 9.70001 12.166C9.92077 12.5768 10.2278 12.935 10.6 13.216C10.0409 13.356 9.49453 13.5428 8.96668 13.7743V13.766ZM12.8833 13.616C12.8833 13.616 12.7333 13.7993 11.775 13.3827C12.8167 13.316 12.9917 13.5577 12.8833 13.6243V13.616Z"
                    fill="#334655"
                  />
                </svg>
                Discovery Materials.PDF
              </div>
              {/* <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M18 12.75H6C5.59 12.75 5.25 12.41 5.25 12C5.25 11.59 5.59 11.25 6 11.25H18C18.41 11.25 18.75 11.59 18.75 12C18.75 12.41 18.41 12.75 18 12.75Z"
                fill="#292D32"
              />
              <path
                d="M12 18.75C11.59 18.75 11.25 18.41 11.25 18V6C11.25 5.59 11.59 5.25 12 5.25C12.41 5.25 12.75 5.59 12.75 6V18C12.75 18.41 12.41 18.75 12 18.75Z"
                fill="#292D32"
              />
            </svg> */}

              {/* <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M12.0001 13.0597L6.53039 18.5294L5.46973 17.4688L10.9394 11.9991L5.46973 6.52941L6.53039 5.46875L12.0001 10.9384L17.4697 5.46875L18.5304 6.52941L13.0607 11.9991L18.5304 17.4688L17.4697 18.5294L12.0001 13.0597Z" fill="#0E0E2C"/>
</svg> */}

              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M12.0001 21.2491C11.8101 21.2491 11.6201 21.1791 11.4701 21.0291L5.40012 14.9591C5.11012 14.6691 5.11012 14.1891 5.40012 13.8991C5.69012 13.6091 6.17012 13.6091 6.46012 13.8991L12.0001 19.4391L17.5401 13.8991C17.8301 13.6091 18.3101 13.6091 18.6001 13.8991C18.8901 14.1891 18.8901 14.6691 18.6001 14.9591L12.5301 21.0291C12.3801 21.1791 12.1901 21.2491 12.0001 21.2491Z"
                  fill="#292D32"
                />
                <path
                  d="M12 21.08C11.59 21.08 11.25 20.74 11.25 20.33V3.5C11.25 3.09 11.59 2.75 12 2.75C12.41 2.75 12.75 3.09 12.75 3.5V20.33C12.75 20.74 12.41 21.08 12 21.08Z"
                  fill="#292D32"
                />
              </svg>
            </div>
            <div className="case-details__inner__accused__attachment__doc case-details__inner__accused__attachment__doc-filled">
              {/* Attach Miscellaneous{" "} */}

              <div>
                <svg
                  width="20"
                  height="21"
                  viewBox="0 0 20 21"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M15.4417 8.00065L10.8333 3.39232C10.7162 3.27507 10.5574 3.20913 10.3917 3.20898H6.66667C6.05888 3.20898 5.47598 3.45043 5.04621 3.8802C4.61644 4.30997 4.375 4.89286 4.375 5.50065V15.5007C4.375 16.1084 4.61644 16.6913 5.04621 17.1211C5.47598 17.5509 6.05888 17.7923 6.66667 17.7923H13.3333C13.9411 17.7923 14.524 17.5509 14.9538 17.1211C15.3836 16.6913 15.625 16.1084 15.625 15.5007V8.41732C15.6184 8.26032 15.553 8.11156 15.4417 8.00065ZM11.0417 5.34232L13.4917 7.79232H11.0417V5.34232ZM13.3333 16.5423H6.66667C6.3904 16.5423 6.12545 16.4326 5.9301 16.2372C5.73475 16.0419 5.625 15.7769 5.625 15.5007V5.50065C5.625 5.22438 5.73475 4.95943 5.9301 4.76408C6.12545 4.56873 6.3904 4.45898 6.66667 4.45898H9.79167V8.41732C9.79382 8.58241 9.86037 8.74013 9.97711 8.85687C10.0939 8.97362 10.2516 9.04016 10.4167 9.04232H14.375V15.5007C14.375 15.7769 14.2653 16.0419 14.0699 16.2372C13.8746 16.4326 13.6096 16.5423 13.3333 16.5423Z"
                    fill="#334655"
                  />
                  <path
                    d="M11.2417 12.8743C10.7293 12.5528 10.3436 12.0641 10.15 11.491C10.3294 10.9548 10.3836 10.3847 10.3083 9.82433C10.2843 9.68316 10.2155 9.55347 10.112 9.45448C10.0085 9.35549 9.87592 9.29244 9.73382 9.27468C9.59172 9.25692 9.44769 9.28538 9.32303 9.35586C9.19837 9.42634 9.09972 9.53508 9.04168 9.66599C8.9468 10.3397 9.01833 11.0263 9.25001 11.666C8.93351 12.4054 8.58029 13.1285 8.19168 13.8327C7.60001 14.166 6.79168 14.666 6.66668 15.241C6.56668 15.7077 7.44168 16.9077 8.93334 14.3077C9.59662 14.0614 10.276 13.861 10.9667 13.7077C11.4772 13.9996 12.0468 14.1733 12.6333 14.216C12.768 14.2195 12.9007 14.1831 13.0148 14.1114C13.1288 14.0397 13.2192 13.9358 13.2744 13.8129C13.3296 13.69 13.3472 13.5536 13.3251 13.4207C13.303 13.2878 13.2421 13.1644 13.15 13.066C12.8 12.7077 11.7583 12.8077 11.2417 12.8743ZM7.25834 15.3743C7.49188 14.9747 7.80028 14.6238 8.16668 14.341C7.60001 15.241 7.25834 15.3993 7.25834 15.3827V15.3743ZM9.69168 9.69933C9.90834 9.69933 9.89168 10.6577 9.74168 10.916C9.62868 10.523 9.61154 10.1086 9.69168 9.70766V9.69933ZM8.96668 13.766C9.24902 13.2509 9.49409 12.7162 9.70001 12.166C9.92077 12.5768 10.2278 12.935 10.6 13.216C10.0409 13.356 9.49453 13.5428 8.96668 13.7743V13.766ZM12.8833 13.616C12.8833 13.616 12.7333 13.7993 11.775 13.3827C12.8167 13.316 12.9917 13.5577 12.8833 13.6243V13.616Z"
                    fill="#334655"
                  />
                </svg>
                Miscellaneous.PDF
              </div>
              {/* <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M18 12.75H6C5.59 12.75 5.25 12.41 5.25 12C5.25 11.59 5.59 11.25 6 11.25H18C18.41 11.25 18.75 11.59 18.75 12C18.75 12.41 18.41 12.75 18 12.75Z"
                fill="#292D32"
              />
              <path
                d="M12 18.75C11.59 18.75 11.25 18.41 11.25 18V6C11.25 5.59 11.59 5.25 12 5.25C12.41 5.25 12.75 5.59 12.75 6V18C12.75 18.41 12.41 18.75 12 18.75Z"
                fill="#292D32"
              />
            </svg> */}

              {/* <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M12.0001 13.0597L6.53039 18.5294L5.46973 17.4688L10.9394 11.9991L5.46973 6.52941L6.53039 5.46875L12.0001 10.9384L17.4697 5.46875L18.5304 6.52941L13.0607 11.9991L18.5304 17.4688L17.4697 18.5294L12.0001 13.0597Z" fill="#0E0E2C"/>
</svg> */}

              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M12.0001 21.2491C11.8101 21.2491 11.6201 21.1791 11.4701 21.0291L5.40012 14.9591C5.11012 14.6691 5.11012 14.1891 5.40012 13.8991C5.69012 13.6091 6.17012 13.6091 6.46012 13.8991L12.0001 19.4391L17.5401 13.8991C17.8301 13.6091 18.3101 13.6091 18.6001 13.8991C18.8901 14.1891 18.8901 14.6691 18.6001 14.9591L12.5301 21.0291C12.3801 21.1791 12.1901 21.2491 12.0001 21.2491Z"
                  fill="#292D32"
                />
                <path
                  d="M12 21.08C11.59 21.08 11.25 20.74 11.25 20.33V3.5C11.25 3.09 11.59 2.75 12 2.75C12.41 2.75 12.75 3.09 12.75 3.5V20.33C12.75 20.74 12.41 21.08 12 21.08Z"
                  fill="#292D32"
                />
              </svg>
            </div>
            <div className="case-details__inner__accused__attachment__doc case-details__inner__accused__attachment__doc-filled">
              {/* Attach Correspondence{" "} */}

              <div>
                <svg
                  width="20"
                  height="21"
                  viewBox="0 0 20 21"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M15.4417 8.00065L10.8333 3.39232C10.7162 3.27507 10.5574 3.20913 10.3917 3.20898H6.66667C6.05888 3.20898 5.47598 3.45043 5.04621 3.8802C4.61644 4.30997 4.375 4.89286 4.375 5.50065V15.5007C4.375 16.1084 4.61644 16.6913 5.04621 17.1211C5.47598 17.5509 6.05888 17.7923 6.66667 17.7923H13.3333C13.9411 17.7923 14.524 17.5509 14.9538 17.1211C15.3836 16.6913 15.625 16.1084 15.625 15.5007V8.41732C15.6184 8.26032 15.553 8.11156 15.4417 8.00065ZM11.0417 5.34232L13.4917 7.79232H11.0417V5.34232ZM13.3333 16.5423H6.66667C6.3904 16.5423 6.12545 16.4326 5.9301 16.2372C5.73475 16.0419 5.625 15.7769 5.625 15.5007V5.50065C5.625 5.22438 5.73475 4.95943 5.9301 4.76408C6.12545 4.56873 6.3904 4.45898 6.66667 4.45898H9.79167V8.41732C9.79382 8.58241 9.86037 8.74013 9.97711 8.85687C10.0939 8.97362 10.2516 9.04016 10.4167 9.04232H14.375V15.5007C14.375 15.7769 14.2653 16.0419 14.0699 16.2372C13.8746 16.4326 13.6096 16.5423 13.3333 16.5423Z"
                    fill="#334655"
                  />
                  <path
                    d="M11.2417 12.8743C10.7293 12.5528 10.3436 12.0641 10.15 11.491C10.3294 10.9548 10.3836 10.3847 10.3083 9.82433C10.2843 9.68316 10.2155 9.55347 10.112 9.45448C10.0085 9.35549 9.87592 9.29244 9.73382 9.27468C9.59172 9.25692 9.44769 9.28538 9.32303 9.35586C9.19837 9.42634 9.09972 9.53508 9.04168 9.66599C8.9468 10.3397 9.01833 11.0263 9.25001 11.666C8.93351 12.4054 8.58029 13.1285 8.19168 13.8327C7.60001 14.166 6.79168 14.666 6.66668 15.241C6.56668 15.7077 7.44168 16.9077 8.93334 14.3077C9.59662 14.0614 10.276 13.861 10.9667 13.7077C11.4772 13.9996 12.0468 14.1733 12.6333 14.216C12.768 14.2195 12.9007 14.1831 13.0148 14.1114C13.1288 14.0397 13.2192 13.9358 13.2744 13.8129C13.3296 13.69 13.3472 13.5536 13.3251 13.4207C13.303 13.2878 13.2421 13.1644 13.15 13.066C12.8 12.7077 11.7583 12.8077 11.2417 12.8743ZM7.25834 15.3743C7.49188 14.9747 7.80028 14.6238 8.16668 14.341C7.60001 15.241 7.25834 15.3993 7.25834 15.3827V15.3743ZM9.69168 9.69933C9.90834 9.69933 9.89168 10.6577 9.74168 10.916C9.62868 10.523 9.61154 10.1086 9.69168 9.70766V9.69933ZM8.96668 13.766C9.24902 13.2509 9.49409 12.7162 9.70001 12.166C9.92077 12.5768 10.2278 12.935 10.6 13.216C10.0409 13.356 9.49453 13.5428 8.96668 13.7743V13.766ZM12.8833 13.616C12.8833 13.616 12.7333 13.7993 11.775 13.3827C12.8167 13.316 12.9917 13.5577 12.8833 13.6243V13.616Z"
                    fill="#334655"
                  />
                </svg>
                Correspondence.PDF
              </div>

              {/* <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M18 12.75H6C5.59 12.75 5.25 12.41 5.25 12C5.25 11.59 5.59 11.25 6 11.25H18C18.41 11.25 18.75 11.59 18.75 12C18.75 12.41 18.41 12.75 18 12.75Z"
                fill="#292D32"
              />
              <path
                d="M12 18.75C11.59 18.75 11.25 18.41 11.25 18V6C11.25 5.59 11.59 5.25 12 5.25C12.41 5.25 12.75 5.59 12.75 6V18C12.75 18.41 12.41 18.75 12 18.75Z"
                fill="#292D32"
              />
            </svg> */}

              {/* <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M12.0001 13.0597L6.53039 18.5294L5.46973 17.4688L10.9394 11.9991L5.46973 6.52941L6.53039 5.46875L12.0001 10.9384L17.4697 5.46875L18.5304 6.52941L13.0607 11.9991L18.5304 17.4688L17.4697 18.5294L12.0001 13.0597Z" fill="#0E0E2C"/>
</svg> */}

              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M12.0001 21.2491C11.8101 21.2491 11.6201 21.1791 11.4701 21.0291L5.40012 14.9591C5.11012 14.6691 5.11012 14.1891 5.40012 13.8991C5.69012 13.6091 6.17012 13.6091 6.46012 13.8991L12.0001 19.4391L17.5401 13.8991C17.8301 13.6091 18.3101 13.6091 18.6001 13.8991C18.8901 14.1891 18.8901 14.6691 18.6001 14.9591L12.5301 21.0291C12.3801 21.1791 12.1901 21.2491 12.0001 21.2491Z"
                  fill="#292D32"
                />
                <path
                  d="M12 21.08C11.59 21.08 11.25 20.74 11.25 20.33V3.5C11.25 3.09 11.59 2.75 12 2.75C12.41 2.75 12.75 3.09 12.75 3.5V20.33C12.75 20.74 12.41 21.08 12 21.08Z"
                  fill="#292D32"
                />
              </svg>
            </div>
            <div className="case-details__inner__accused__attachment__doc case-details__inner__accused__attachment__doc-filled">
              {/* Attach Audio{" "} */}

              <div>
                <svg
                  width="20"
                  height="21"
                  viewBox="0 0 20 21"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M12.5 19.4577H7.50002C2.97502 19.4577 1.04169 17.5243 1.04169 12.9993V7.99935C1.04169 3.47435 2.97502 1.54102 7.50002 1.54102H12.5C17.025 1.54102 18.9584 3.47435 18.9584 7.99935V12.9993C18.9584 17.5243 17.025 19.4577 12.5 19.4577ZM7.50002 2.79102C3.65835 2.79102 2.29169 4.15768 2.29169 7.99935V12.9993C2.29169 16.841 3.65835 18.2077 7.50002 18.2077H12.5C16.3417 18.2077 17.7084 16.841 17.7084 12.9993V7.99935C17.7084 4.15768 16.3417 2.79102 12.5 2.79102H7.50002Z"
                    fill="#292D32"
                  />
                  <path
                    d="M8.01666 15.5411C6.7 15.5411 5.625 14.4661 5.625 13.1495C5.625 11.8328 6.7 10.7578 8.01666 10.7578C9.33333 10.7578 10.4083 11.8328 10.4083 13.1495C10.4083 14.4661 9.33333 15.5411 8.01666 15.5411ZM8.01666 12.0162C7.39166 12.0162 6.875 12.5245 6.875 13.1578C6.875 13.7828 7.38333 14.2995 8.01666 14.2995C8.64166 14.2995 9.15833 13.7911 9.15833 13.1578C9.15833 12.5245 8.64166 12.0162 8.01666 12.0162Z"
                    fill="#292D32"
                  />
                  <path
                    d="M9.78351 13.7746C9.44184 13.7746 9.15851 13.4913 9.15851 13.1496V6.97461C9.15851 6.63294 9.44184 6.34961 9.78351 6.34961C10.1252 6.34961 10.4085 6.63294 10.4085 6.97461V13.1496C10.4085 13.4996 10.1252 13.7746 9.78351 13.7746Z"
                    fill="#292D32"
                  />
                  <path
                    d="M12.9335 10.0244C12.7585 10.0244 12.5752 9.99111 12.3918 9.93278L10.4419 9.28275C9.70854 9.04109 9.15851 8.27446 9.15851 7.49946V6.9828C9.15851 6.4578 9.3752 5.99946 9.75853 5.72446C10.1419 5.44946 10.6418 5.38277 11.1418 5.54944L13.0919 6.19946C13.8252 6.44113 14.3752 7.20776 14.3752 7.98276V8.49942C14.3752 9.02442 14.1585 9.48276 13.7752 9.75776C13.5335 9.93276 13.2419 10.0244 12.9335 10.0244ZM10.5919 6.70778C10.5502 6.70778 10.5085 6.71609 10.4835 6.73276C10.4335 6.76609 10.4002 6.85779 10.4002 6.97446V7.49112C10.4002 7.72445 10.6168 8.01609 10.8335 8.09109L12.7835 8.74112C12.8919 8.77445 12.9919 8.77445 13.0419 8.74112C13.0919 8.70779 13.1252 8.61609 13.1252 8.49942V7.98276C13.1252 7.74942 12.9085 7.45778 12.6918 7.38278L10.7419 6.73276C10.6919 6.71609 10.6335 6.70778 10.5919 6.70778Z"
                    fill="#292D32"
                  />
                </svg>
                Audio.Mp3
              </div>

              {/* <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M18 12.75H6C5.59 12.75 5.25 12.41 5.25 12C5.25 11.59 5.59 11.25 6 11.25H18C18.41 11.25 18.75 11.59 18.75 12C18.75 12.41 18.41 12.75 18 12.75Z"
                fill="#292D32"
              />
              <path
                d="M12 18.75C11.59 18.75 11.25 18.41 11.25 18V6C11.25 5.59 11.59 5.25 12 5.25C12.41 5.25 12.75 5.59 12.75 6V18C12.75 18.41 12.41 18.75 12 18.75Z"
                fill="#292D32"
              />
            </svg> */}

              {/* <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M12.0001 13.0597L6.53039 18.5294L5.46973 17.4688L10.9394 11.9991L5.46973 6.52941L6.53039 5.46875L12.0001 10.9384L17.4697 5.46875L18.5304 6.52941L13.0607 11.9991L18.5304 17.4688L17.4697 18.5294L12.0001 13.0597Z" fill="#0E0E2C"/>
</svg> */}

              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M12.0001 21.2491C11.8101 21.2491 11.6201 21.1791 11.4701 21.0291L5.40012 14.9591C5.11012 14.6691 5.11012 14.1891 5.40012 13.8991C5.69012 13.6091 6.17012 13.6091 6.46012 13.8991L12.0001 19.4391L17.5401 13.8991C17.8301 13.6091 18.3101 13.6091 18.6001 13.8991C18.8901 14.1891 18.8901 14.6691 18.6001 14.9591L12.5301 21.0291C12.3801 21.1791 12.1901 21.2491 12.0001 21.2491Z"
                  fill="#292D32"
                />
                <path
                  d="M12 21.08C11.59 21.08 11.25 20.74 11.25 20.33V3.5C11.25 3.09 11.59 2.75 12 2.75C12.41 2.75 12.75 3.09 12.75 3.5V20.33C12.75 20.74 12.41 21.08 12 21.08Z"
                  fill="#292D32"
                />
              </svg>
            </div>
            <div className="case-details__inner__accused__attachment__doc case-details__inner__accused__attachment__doc-filled">
              {/* Attach Video{" "} */}

              <div>
                <svg
                  width="20"
                  height="21"
                  viewBox="0 0 20 21"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M10.7334 17.8839H5.67502C2.71669 17.8839 1.66669 15.8089 1.66669 13.8755V7.12552C1.66669 4.24219 2.79169 3.11719 5.67502 3.11719H10.7334C13.6167 3.11719 14.7417 4.24219 14.7417 7.12552V13.8755C14.7417 16.7589 13.6167 17.8839 10.7334 17.8839ZM5.67502 4.38385C3.50002 4.38385 2.93335 4.95052 2.93335 7.12552V13.8755C2.93335 14.9005 3.29169 16.6172 5.67502 16.6172H10.7334C12.9084 16.6172 13.475 16.0505 13.475 13.8755V7.12552C13.475 4.95052 12.9084 4.38385 10.7334 4.38385H5.67502Z"
                    fill="#292D32"
                  />
                  <path
                    d="M17.3166 15.5911C16.9583 15.5911 16.4999 15.4744 15.9749 15.1078L13.7499 13.5494C13.5833 13.4328 13.4833 13.2411 13.4833 13.0328V7.96609C13.4833 7.75776 13.5833 7.56609 13.7499 7.44943L15.9749 5.89109C16.9666 5.19943 17.6916 5.39943 18.0333 5.57443C18.3749 5.75776 18.9583 6.23276 18.9583 7.44109V13.5494C18.9583 14.7578 18.3749 15.2411 18.0333 15.4161C17.8749 15.5078 17.6249 15.5911 17.3166 15.5911ZM14.7416 12.6994L16.6999 14.0661C17.0749 14.3244 17.3416 14.3494 17.4499 14.2911C17.5666 14.2328 17.6916 13.9994 17.6916 13.5494V7.44943C17.6916 6.99109 17.5583 6.76609 17.4499 6.70776C17.3416 6.64943 17.0749 6.67443 16.6999 6.93276L14.7416 8.29943V12.6994Z"
                    fill="#292D32"
                  />
                  <path
                    d="M9.58331 10.291C8.54998 10.291 7.70831 9.44935 7.70831 8.41602C7.70831 7.38268 8.54998 6.54102 9.58331 6.54102C10.6166 6.54102 11.4583 7.38268 11.4583 8.41602C11.4583 9.44935 10.6166 10.291 9.58331 10.291ZM9.58331 7.79102C9.24165 7.79102 8.95831 8.07435 8.95831 8.41602C8.95831 8.75768 9.24165 9.04102 9.58331 9.04102C9.92498 9.04102 10.2083 8.75768 10.2083 8.41602C10.2083 8.07435 9.92498 7.79102 9.58331 7.79102Z"
                    fill="#292D32"
                  />
                </svg>
                Video.Mp4
              </div>
              {/* <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M18 12.75H6C5.59 12.75 5.25 12.41 5.25 12C5.25 11.59 5.59 11.25 6 11.25H18C18.41 11.25 18.75 11.59 18.75 12C18.75 12.41 18.41 12.75 18 12.75Z"
                fill="#292D32"
              />
              <path
                d="M12 18.75C11.59 18.75 11.25 18.41 11.25 18V6C11.25 5.59 11.59 5.25 12 5.25C12.41 5.25 12.75 5.59 12.75 6V18C12.75 18.41 12.41 18.75 12 18.75Z"
                fill="#292D32"
              />
            </svg> */}

              {/* <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path fill-rule="evenodd" clip-rule="evenodd" d="M12.0001 13.0597L6.53039 18.5294L5.46973 17.4688L10.9394 11.9991L5.46973 6.52941L6.53039 5.46875L12.0001 10.9384L17.4697 5.46875L18.5304 6.52941L13.0607 11.9991L18.5304 17.4688L17.4697 18.5294L12.0001 13.0597Z" fill="#0E0E2C"/>
</svg> */}

              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M12.0001 21.2491C11.8101 21.2491 11.6201 21.1791 11.4701 21.0291L5.40012 14.9591C5.11012 14.6691 5.11012 14.1891 5.40012 13.8991C5.69012 13.6091 6.17012 13.6091 6.46012 13.8991L12.0001 19.4391L17.5401 13.8991C17.8301 13.6091 18.3101 13.6091 18.6001 13.8991C18.8901 14.1891 18.8901 14.6691 18.6001 14.9591L12.5301 21.0291C12.3801 21.1791 12.1901 21.2491 12.0001 21.2491Z"
                  fill="#292D32"
                />
                <path
                  d="M12 21.08C11.59 21.08 11.25 20.74 11.25 20.33V3.5C11.25 3.09 11.59 2.75 12 2.75C12.41 2.75 12.75 3.09 12.75 3.5V20.33C12.75 20.74 12.41 21.08 12 21.08Z"
                  fill="#292D32"
                />
              </svg>
            </div>
          </div>
        </div>

        <div className="case-details__mugshot">
          <div className="case-details__mugshot__title">Mugshot</div>
          <div className="case-details__mugshot__subtitle"></div>
        </div>
        <div className="case-details__mugshots">
          <Image
            className="case-details__accused__bio__img"
            alt=""
            src="/assets/Image (2).png"
            width={349}
            height={273}
            style={{
              objectFit: "cover",
              borderRadius: "12px",
              minWidth: "349px",
            }}
          />
          <Image
            className="case-details__accused__bio__img"
            alt=""
            src="/assets/Image (2).png"
            width={349}
            height={273}
            style={{
              objectFit: "cover",
              borderRadius: "12px",
              minWidth: "349px",
            }}
          />
          <Image
            className="case-details__accused__bio__img"
            alt=""
            src="/assets/Image (2).png"
            width={349}
            height={273}
            style={{
              objectFit: "cover",
              borderRadius: "12px",
              minWidth: "349px",
            }}
          />
          <Image
            className="case-details__accused__bio__img"
            alt=""
            src="/assets/Image (2).png"
            width={349}
            height={273}
            style={{
              objectFit: "cover",
              borderRadius: "12px",
              minWidth: "349px",
            }}
          />
        </div>

        {/* <div className="case-details__inner__button-group">
        <button>Cancel</button>
        <button>Save File</button>
      </div> */}
      </form>

      {modalOpen && (
        <div className="case-details__modal" onClick={handleModalClose}>
          {modalStage === 1 || modalStage === 2 ? (
            <div className="case-details__modal__inner">
              <svg
                className="case-details__modal__inner__icon"
                width="80"
                height="81"
                viewBox="0 0 80 81"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <rect
                  y="0.5"
                  width="80"
                  height="80"
                  rx="40"
                  fill="url(#paint0_linear_7644_138298)"
                />
                <path
                  d="M52.0942 31.4727C49.9475 31.2593 47.8009 31.0993 45.6409 30.9793V30.966L45.3475 29.2327C45.1475 28.006 44.8542 26.166 41.7342 26.166H38.2409C35.1342 26.166 34.8409 27.926 34.6275 29.2193L34.3475 30.926C33.1075 31.006 31.8675 31.086 30.6275 31.206L27.9075 31.4727C27.3475 31.526 26.9475 32.0193 27.0009 32.566C27.0542 33.1127 27.5342 33.5127 28.0942 33.4593L30.8142 33.1927C37.8009 32.4993 44.8409 32.766 51.9075 33.4727C51.9475 33.4727 51.9742 33.4727 52.0142 33.4727C52.5209 33.4727 52.9609 33.086 53.0142 32.566C53.0542 32.0193 52.6542 31.526 52.0942 31.4727Z"
                  fill="#ED1651"
                />
                <path
                  d="M49.6403 35.354C49.3203 35.0207 48.8803 34.834 48.427 34.834H31.5737C31.1203 34.834 30.667 35.0207 30.3603 35.354C30.0537 35.6873 29.8803 36.1407 29.907 36.6073L30.7337 50.2873C30.8803 52.314 31.067 54.8473 35.7203 54.8473H44.2803C48.9337 54.8473 49.1203 52.3273 49.267 50.2873L50.0937 36.6207C50.1203 36.1407 49.947 35.6873 49.6403 35.354ZM42.2136 48.1673H37.7737C37.227 48.1673 36.7737 47.714 36.7737 47.1673C36.7737 46.6207 37.227 46.1673 37.7737 46.1673H42.2136C42.7603 46.1673 43.2136 46.6207 43.2136 47.1673C43.2136 47.714 42.7603 48.1673 42.2136 48.1673ZM43.3337 42.834H36.667C36.1203 42.834 35.667 42.3807 35.667 41.834C35.667 41.2873 36.1203 40.834 36.667 40.834H43.3337C43.8803 40.834 44.3337 41.2873 44.3337 41.834C44.3337 42.3807 43.8803 42.834 43.3337 42.834Z"
                  fill="#ED1651"
                />
                <defs>
                  <linearGradient
                    id="paint0_linear_7644_138298"
                    x1="40"
                    y1="0.5"
                    x2="40"
                    y2="86.5"
                    gradientUnits="userSpaceOnUse"
                  >
                    <stop stop-color="#FF2B65" stop-opacity="0.2" />
                    <stop offset="1" stop-color="#ED1651" stop-opacity="0" />
                  </linearGradient>
                </defs>
              </svg>

              {modalStage === 1 && (
                <div className="case-details__modal__inner__title">
                  to Expunge Case No PF00458
                </div>
              )}
              {modalStage === 1 && (
                <div className="case-details__modal__inner__title">
                  <span>
                    Please upload Certified Court Order for Expungement
                  </span>
                </div>
              )}

              {modalStage === 1 && (
                <label>
                  <div className="case-details__modal__inner__upload">
                    <svg
                      width="60"
                      height="61"
                      viewBox="0 0 60 61"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <circle cx="30" cy="30.5" r="30" fill="#E5FFE7" />
                    </svg>

                    <div className="case-details__modal__inner__upload__title">
                      Drag & drop files or <span>Browse</span>
                    </div>
                    <div className="case-details__modal__inner__upload__subtitle">
                      Supported formates: JPEG, PNG, PDF
                    </div>
                  </div>

                  <input
                    type="file"
                    name=""
                    id=""
                    hidden
                    ref={mediaRef}
                    accept=".pdf"
                    onChange={(e) => handleUpload(e.target.files[0])}
                  />
                </label>
              )}

              {modalStage === 1 && (
                <>
                  {loading && (
                    <div className="case-details__modal__inner__label">
                      Uploading
                    </div>
                  )}
                  {loading && (
                    <div className="case-details__modal__inner__uploading">
                      <div>Court-Order Expungement PF00458.PDF</div>

                      <svg
                        width="17"
                        height="17"
                        viewBox="0 0 17 17"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M8.5 16.5C6.37833 16.5 4.34331 15.6571 2.84315 14.1568C1.3429 12.6565 0.5 10.6217 0.5 8.5C0.5 6.37833 1.3429 4.34332 2.84315 2.84315C4.34353 1.3429 6.37833 0.5 8.5 0.5C10.6217 0.5 12.6567 1.3429 14.1568 2.84315C15.6571 4.34353 16.5 6.37833 16.5 8.5C16.4974 10.621 15.6538 12.6545 14.154 14.154C12.6543 15.6538 10.6207 16.4974 8.5 16.5ZM11.5851 6.44242V6.4423C11.7254 6.30678 11.8053 6.12063 11.8071 5.92558C11.8088 5.73053 11.7321 5.54292 11.5941 5.40496C11.4562 5.26701 11.2685 5.19031 11.0735 5.19202C10.8784 5.19372 10.6923 5.27371 10.5568 5.41397L8.5 7.47149L6.44323 5.41397C6.25853 5.2356 5.99346 5.16779 5.7457 5.23572C5.49805 5.30366 5.3046 5.49713 5.23665 5.74477C5.16871 5.99254 5.23653 6.2576 5.4149 6.4423L7.47167 8.49983L5.4149 10.5573C5.27464 10.6929 5.19465 10.879 5.19294 11.0741C5.19124 11.2691 5.26794 11.4567 5.40589 11.5947C5.54383 11.7326 5.73145 11.8093 5.9265 11.8076C6.12155 11.8059 6.3077 11.7259 6.44322 11.5857L8.49999 9.52815L10.5568 11.5857C10.6923 11.7259 10.8784 11.8059 11.0735 11.8076C11.2685 11.8093 11.4561 11.7326 11.5941 11.5947C11.7321 11.4567 11.8088 11.2691 11.807 11.0741C11.8053 10.879 11.7253 10.6929 11.5851 10.5573L9.52832 8.49983L11.5851 6.44242Z"
                          fill="#E6E6E6"
                        />
                      </svg>
                    </div>
                  )}
                  {!!doc && (
                    <div className="case-details__modal__inner__label">
                      Uploaded
                    </div>
                  )}
                  {!!doc && (
                    <div className="case-details__modal__inner__uploaded">
                      <div>Court-Order Expungement PF00458.PDF</div>

                      <svg
                        width="17"
                        height="17"
                        viewBox="0 0 17 17"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        className="pointer"
                        onClick={() => setDoc(null)}
                      >
                        <circle cx="8.5" cy="8.5" r="8" fill="#FFF3F3" />
                        <path
                          d="M11.9769 5.64517H10.9774C10.964 5.64054 10.9501 5.63753 10.936 5.63614H9.87174V4.75348C9.8718 4.68714 9.8458 4.62346 9.79945 4.57608C9.75304 4.52871 9.68995 4.50138 9.62369 4.5H7.10672C7.03948 4.5 6.97501 4.52667 6.92752 4.57422C6.87997 4.62177 6.85324 4.68624 6.85324 4.75348V5.63614H5.79439C5.7803 5.63753 5.7664 5.64054 5.75303 5.64517H4.75347C4.66288 5.64517 4.57921 5.69345 4.53395 5.77188C4.48868 5.85031 4.48868 5.94691 4.53395 6.02535C4.57921 6.10378 4.66288 6.15211 4.75347 6.15211H5.56783L6.22585 12.2735C6.2329 12.3358 6.26263 12.3933 6.3094 12.4351C6.35611 12.4769 6.41661 12.4999 6.47932 12.5H10.2546C10.3167 12.4991 10.3763 12.4756 10.4223 12.4339C10.4683 12.3922 10.4975 12.3352 10.5045 12.2735L11.1625 6.16105H11.9769C12.0675 6.16105 12.1511 6.11277 12.1964 6.03434C12.2417 5.95591 12.2417 5.85924 12.1964 5.78087C12.1511 5.70244 12.0675 5.6541 11.9769 5.6541L11.9769 5.64517ZM7.36566 5.00695H9.36472V5.63614H7.36566V5.00695ZM10.0245 11.9894H6.70582L6.07663 6.16111H10.6537L10.0245 11.9894Z"
                          fill="#E41D1D"
                        />
                        <path
                          d="M8.36675 11.3957C8.43399 11.3957 8.49845 11.369 8.54601 11.3215C8.59356 11.2739 8.62023 11.2095 8.62023 11.1422V7.00742C8.62023 6.91683 8.57195 6.83316 8.49352 6.7879C8.41509 6.74257 8.31842 6.74257 8.23999 6.7879C8.16156 6.83317 8.11328 6.91683 8.11328 7.00742V11.1422C8.11328 11.2095 8.13995 11.2739 8.1875 11.3215C8.23505 11.369 8.29952 11.3957 8.36676 11.3957H8.36675Z"
                          fill="#E41D1D"
                        />
                        <path
                          d="M7.3674 11.3944H7.37817C7.44511 11.3917 7.50819 11.3625 7.5537 11.3133C7.59915 11.2641 7.62334 11.1989 7.62088 11.132L7.46805 6.99719L7.46811 6.99713C7.46672 6.92941 7.43777 6.86512 7.38787 6.81926C7.33791 6.77345 7.27146 6.75004 7.2038 6.75443C7.13687 6.75726 7.07379 6.78645 7.02834 6.83563C6.98283 6.88481 6.95869 6.95 6.9611 7.01694L7.11393 11.1517C7.11712 11.2169 7.14517 11.2784 7.19236 11.3236C7.23949 11.3687 7.30215 11.3941 7.3674 11.3944L7.3674 11.3944Z"
                          fill="#E41D1D"
                        />
                        <path
                          d="M9.35421 11.3931H9.36318C9.42999 11.3946 9.4947 11.3698 9.54345 11.3241C9.59215 11.2784 9.62104 11.2154 9.62387 11.1486L9.7767 7.01379H9.77664C9.77911 6.94686 9.75491 6.88167 9.70946 6.83249C9.66396 6.78331 9.60088 6.75412 9.53394 6.75129C9.4655 6.74424 9.39724 6.76633 9.34589 6.8122C9.29455 6.85801 9.26493 6.92338 9.26427 6.99224L9.10964 11.127H9.1097C9.10627 11.1949 9.13022 11.2613 9.17621 11.3113C9.22214 11.3613 9.2863 11.3908 9.35421 11.3931L9.35421 11.3931Z"
                          fill="#E41D1D"
                        />
                      </svg>
                    </div>
                  )}
                </>
              )}

              {modalStage === 1 && (
                <button
                  className="case-details__modal__inner__button"
                  onClick={handleStage1}
                >
                  Yes
                </button>
              )}

              {modalStage === 2 && (
                <>
                  <div className="case-details__modal__inner__title">
                    <span>expungement Review</span>
                  </div>
                  <div className="case-details__modal__inner__subtitle">
                    Expungement by: <span>Alaba Adenuga(FM56898324)</span>
                  </div>
                  <div className="case-details__modal__inner__subtitle">
                    Title: <span>Director of Legal Records</span>
                  </div>
                  <div className="case-details__modal__inner__subtitle">
                    Date: <span>06-06-2024</span>
                  </div>
                  <div className="case-details__modal__inner__subtitle">
                    Agency: <span>Federal Ministry of Justice HQ Abuja</span>
                  </div>
                  <div className="case-details__modal__inner__file-name">
                    Expunge case file: <span>PF00458</span>
                  </div>

                  <div className="case-details__modal__inner__question">
                    Are you sure you want to <br /> proceed?
                  </div>

                  <div className="case-details__modal__inner__label">
                    Uploaded
                  </div>
                  <div className="case-details__modal__inner__uploaded">
                    <div>Court-Order Expungement PF00458.PDF</div>

                    <div>View</div>
                  </div>

                  <button
                    className="case-details__modal__inner__button"
                    onClick={handleStage2}
                  >
                    Yes
                  </button>
                </>
              )}
            </div>
          ) : (
            ""
          )}

          {modalStage === 3 && (
            <form className="case-details__modal__otp"
            
            onSubmit={handleStage3}>
              <div className="case-details__modal__otp__title">OTP</div>
              <div className="case-details__modal__otp__subtitle">
                Enter the OTP sent to Alaba.Adenuga@fmoj.gov.ng
              </div>
              <div className="case-details__modal__otp__otp-group">
                <input type="text" required maxLength={1} />
                <input type="text" required maxLength={1} />
                <input type="text" required maxLength={1} />
                <input type="text" required maxLength={1} />
              </div>
              <div className="case-details__modal__otp__request">
                Request a new code
              </div>

              <button className="case-details__modal__otp__button" type="submit">
                Proceed
              </button>
            </form>
          )}

       {modalStage === 4 &&    <div className="case-details__modal__exp-notif pointer" onClick={handleStage4}>
<svg width="42" height="42" viewBox="0 0 42 42" fill="none" xmlns="http://www.w3.org/2000/svg">
<circle cx="21" cy="21" r="21" fill="#FFC6D6"/>
<path d="M30.0697 14.23C28.4597 14.07 26.8497 13.95 25.2297 13.86V13.85L25.0097 12.55C24.8597 11.63 24.6397 10.25 22.2997 10.25H19.6797C17.3497 10.25 17.1297 11.57 16.9697 12.54L16.7597 13.82C15.8297 13.88 14.8997 13.94 13.9697 14.03L11.9297 14.23C11.5097 14.27 11.2097 14.64 11.2497 15.05C11.2897 15.46 11.6497 15.76 12.0697 15.72L14.1097 15.52C19.3497 15 24.6297 15.2 29.9297 15.73C29.9597 15.73 29.9797 15.73 30.0097 15.73C30.3897 15.73 30.7197 15.44 30.7597 15.05C30.7897 14.64 30.4897 14.27 30.0697 14.23Z" fill="#ED1651"/>
<path d="M28.2297 17.14C27.9897 16.89 27.6597 16.75 27.3197 16.75H14.6797C14.3397 16.75 13.9997 16.89 13.7697 17.14C13.5397 17.39 13.4097 17.73 13.4297 18.08L14.0497 28.34C14.1597 29.86 14.2997 31.76 17.7897 31.76H24.2097C27.6997 31.76 27.8398 29.87 27.9497 28.34L28.5697 18.09C28.5897 17.73 28.4597 17.39 28.2297 17.14ZM22.6597 26.75H19.3297C18.9197 26.75 18.5797 26.41 18.5797 26C18.5797 25.59 18.9197 25.25 19.3297 25.25H22.6597C23.0697 25.25 23.4097 25.59 23.4097 26C23.4097 26.41 23.0697 26.75 22.6597 26.75ZM23.4997 22.75H18.4997C18.0897 22.75 17.7497 22.41 17.7497 22C17.7497 21.59 18.0897 21.25 18.4997 21.25H23.4997C23.9097 21.25 24.2497 21.59 24.2497 22C24.2497 22.41 23.9097 22.75 23.4997 22.75Z" fill="#ED1651"/>
</svg>

<div>Case File <span>PF00458 Expunged</span></div>
</div>}
        </div>
      )}
    </div>
  );
}
